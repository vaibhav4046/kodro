...
# Python install required. The Tk fallback ships from kodro.spec.
import os
...
datas = [
    (f"{ROOT}/src/kodro/lessons/library", "kodro/lessons/library"),
    # The vendored web design (HTML/CSS/JS/JSX + React/Babel + fonts).
    (f"{ROOT}/src/kodro/assets/web", "kodro/assets/web"),
]
...
hiddenimports = ["kodro.web", "kodro.web.app", "kodro.web.__main__"]
...
a = Analysis(
    [f"{ROOT}/src/kodro/web/_pyi_entry.py"],
    pathex=[f"{ROOT}/src"],
    binaries=binaries,
    datas=datas,
...
exe = EXE(
    ...
    icon=f"{ROOT}/src/kodro/assets/icon.ico",
)
