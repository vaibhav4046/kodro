...
- Test layers: scripts/qa_interpreter.mjs (21/21), scripts/qa_ui.mjs (6/6 flows + rover-moved BEHAVIOUR assert), scripts/qa_vibe.mjs (8/8 runnable), pytest (854), bundle-eval (3).
- Decompose progress (all net-verified, verbatim, zero behaviour drift): app.jsx 2320 -> 2051. Modules: app-data.jsx (EXAMPLES/LED_COLORS/BLOCK_DEFS/TWEAK_DEFAULTS/ORBIT_SVG), hooks.jsx (useAiStatus 5bd0c05, useResizers 06aced6, useBlocks c3cf216, useReview 01038cf). VOICE skipped: too cross-wired (voiceGender feeds vibeSend + 3 sites) to move verbatim -> the safe verbatim-extraction lane is now largely EXHAUSTED.
