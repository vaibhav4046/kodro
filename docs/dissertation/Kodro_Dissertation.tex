...
Alongside the Python suite, a separate interpreter QA harness drives the shipped interpreter with the real kinematics and the wall ray, and asserts the command semantics, every bundled example program including the three showcases, and a set of product-coherence checks. It passes at one hundred and fifty six of one hundred and fifty six. Every example terminates, moves, stays inside the arena, never hits a wall and never throws, and the command semantics, the two motion call styles, the turn, the speed clamp, the guarded huge exponent, the loops and the sensor reads, are each asserted, as are the interpreter diagnostics, the honesty of the design-check command list and the absence of emoji glyphs from the interface chrome. This harness is the evidence that the user-facing program surface behaves, independently of the Python engine.
...
\texttt{gemma3:4b} & 0.14 & 0.00 & 0.40 & 0.46 \\
\texttt{gemma3:1b} & 0.02 & 0.00 & 0.40 & 0.18 \\
\texttt{llama3.2:3b} & 0.00 & 0.00 & 0.00 & 0.20 \\
