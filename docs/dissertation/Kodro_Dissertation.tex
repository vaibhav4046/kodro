...
\begin{itemize}
\item an offline-by-default desktop application that imports a measured robot specification, or lets a non-expert assemble a build from a parts catalogue, and derives its mass, closed-form top speed, battery life and exact runnable command set.
\item a three-tier fidelity disclosure, honoured, approximated and not simulated, surfaced as per-figure badges, a realism dashboard and a per-robot verification report, so an approximation is never dressed as a certified result.
\item one shared motion model, closed forms and constants mirrored in JavaScript and Python and locked together by a constants hash, a golden-trace corpus and a formula-parity test, so the visible studio and the tested engine cannot silently drift.
\item two ways to program one robot, a Python-subset interpreter and a blocks palette that compiles to the same Python, so the meaning of a behaviour does not depend on how it was written.
\item a build-aware assistant and code reviewer that receive the fitted command set, normalise known call styles and withhold invalid output after bounded repair attempts, with a deterministic path when no model is installed.
\item a validation layer that runs a program across seventeen named mission sites among moving agents and reports what happened, with an experience memory of reflections and reusable skills available to later sessions without retraining.
\item a grounding metric and seeded benchmark harness, KodroBench, that measure by syntax-tree analysis whether a generated program stays inside the build's fitted-command set, with a leaderboard generated from the machine-readable run record.
\item a development process recorded in a decision log and a test-evidence log and gated by continuous integration, which makes the work reproducible and gives the reflection in this dissertation a factual basis.
\end{itemize}

None of this is a new algorithm: retrieval grounding, sandboxed gates, experience memory, skill libraries and domain randomisation are each known. The contribution is their implemented combination, specification import, fidelity disclosure, shared closed forms, build-aware generation and deterministic rejection, in one early design and learning tool. The memory mechanism is demonstrated; a causal improvement from it is not.

\section{Report structure}
Chapter~2 reviews the literature the design follows from; Chapter~3 states the requirements, the intended users and the central analysis; Chapter~4 the architecture and the key design decisions, chief among them the robot specification as the single source of truth and the deterministic gate between a generated program and the world; Chapter~5 the implementation against the real code; Chapter~6 the evaluation, separating what was measured from what is planned; Chapter~7 the limitations; Chapter~8 ethics, professional issues and reflection; and Chapter~9 the conclusion and roadmap.
...
\looseness=-1 The oldest line is constructionism, the durable finding that people learn most deeply when they build something external that they can watch behave and then debug \citep{papert1980}. The important word is \emph{watch}: a learner who can see the consequence of a change, and change it again, is in a tight feedback loop that abstract instruction cannot match, and Kodro is built around exactly that loop: assemble a robot, write a behaviour, run it, watch it behave in a world, adjust. The blocks palette compiles to the same Python the text editor runs, so moving from blocks to text changes the surface and not the tool, and the learner never throws away what they understood in order to progress, a deliberate echo of the constructionist position that the medium should not get in the way of the idea.
...
\looseness=-1 Official product documentation supports the segmentation rather than any claim that alternatives are deficient: Gazebo describes high-fidelity physics, rendering and sensor models with plugin and messaging entry points \citep{gazebo2026}; Webots describes a desktop simulator for modelling, programming and validating robots that expects some robotics and programming knowledge \citep{webots2026}; VEXcode VR deliberately provides a preconfigured virtual robot in a browser \citep{vex2026}; and Open Roberta centres drag-and-drop programming of supported robots and boards \citep{openroberta2026}. Kodro's differentiator is the workflow it integrates: start with a catalogue or measured specification, expose the resulting command set, test code in a lightweight world, and label what the model does not simulate; optional cloud keys are a connected mode outside the offline guarantee.

A second argument for the offline design is stronger than the usual privacy and cost arguments: a hosted educational tool depends on somebody continuing to pay for the hosting, and that dependency has a documented failure mode. Trinket, the embedded Python widget used across a great many United Kingdom Key Stage 3 and GCSE teaching resources, announced that it will shut down on 31 August 2026, eleven days before this dissertation is due \citep{trinket2026}. The announcement is explicit that publishing the source does not preserve the service: the code is open, the site still goes dark, every resource that embedded a trinket loses its interactive element, and every pupil loses saved work not exported in time.

\looseness=-1 The point is not that one company withdrew a free tier; it is that the architecture decides whether a teaching tool can be withdrawn at all. A tool whose lessons, marking, world and pupil record live in files on the machine cannot be switched off by a third party, cannot lose a pupil's work to an account deletion, and cannot be broken by a change to an interface it does not call, which is why the zero-cost constraint is worth accepting. The honest comparison with a hosted competitor is lifetime against lifetime, not feature against feature, and this project claims only the former.
...
\begin{itemize}
\item \textbf{Offline by default.} No account, no paid interface, and no data leaving the machine on any default path: the only networked call the core makes is to a local model server on loopback, and the platform is fully functional without it. A user may explicitly connect their own cloud model key, in which case only their prompt is sent, and only to the provider they chose; the offline default is never weakened.
...
\section{Summary}
This project set out to build a free, offline-first proving ground in which a builder imports a robot specification, or a non-expert designs one, programs it through text or blocks and tests the design in an agent-populated simulation before buying hardware. The artefact delivers specification-driven behaviour, three-tier fidelity disclosure, mirrored and conformance-gated closed forms, sandboxed execution and scoring, named mission sites, a classroom path, build-aware assistance and deterministic rejection, behind a default Simple three-stage Design, Prove and Build journey with an opt-in Expert surface. At the commit recorded in \texttt{docs/eval/test\_suite.json}, all 1,641 Python tests, 180 interpreter checks, twenty seeded Prove runs and the web gates pass, with no Tk skip in that run, and an intentionally broken controller fails. The local-Ollama runs are executable evidence rather than user evidence; the measured renderer meets the 240 Hz work budget on the host's integrated GPU and misses it under the disclosed software-rasterised floor; and the reflection and skill stores work as memory mechanisms, but objective O6 remains incomplete because no controlled evaluation shows that memory reduces iterations.

One sentence needs to stay narrower than it wants to be. The simulation is built to reduce early design uncertainty: it demonstrably rejects programs a build cannot run, marks a run against the world that was on screen and reports a spread rather than one tidy figure. Whether that reduces a real builder's uncertainty is a claim about a person, and no person was measured. The platform does not reproduce or certify a physical robot, and it has not been shown to change what a builder decides. Figure~\ref{fig:closure} closes each objective and research question from Chapter~1 against the section that carries its evidence.
...
\midrule
O1 & Met & The specification drives motion, battery and command availability: Sections~\ref{sec:spec}, \ref{sec:shared-motion} and~\ref{sec:autoverify}. \\
O2 & Met, after repair & Fidelity tiers by construction (Section~\ref{sec:fidelity}) and a per-robot report (Section~\ref{sec:verreport}); two badge defects found by the panel were fixed (Section~\ref{sec:panel}) and the honesty gate holds the boundary (Section~\ref{sec:autoverify}). \\
...
O5 & Met & Grounded local assistant with a deterministic fallback (Section~\ref{sec:assistant}); the residual invention risk is measured rather than assumed (Section~\ref{sec:kodrobench}). \\
...
O9 & Met on the host only & The renderer budget is met on the integrated GPU and missed on the software floor (Sections~\ref{sec:renderer} and~\ref{sec:framerate}). \\
O10 & Met & Decision log, evidence log and continuous integration (Section~\ref{sec:engineering}); artefact provenance (Section~\ref{sec:evalstrategy}). \\
...
RQ2 & Yes, engine-scoped & Syntax-tree validation flagged every invented symbol that prompt grounding had let through, offline (Section~\ref{sec:kodrobench}); whether a call is refused at run time depends on the engine. \\
...
RQ4 & Yes & Every lesson's worked answer must score full marks through both markers on every change (Section~\ref{sec:classroom-gates}); agreement is not validity (Section~\ref{sec:threats}). \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
\item \textbf{Wire the imported specification through the Python engine}, so its sensor rays honour an imported sensor's mount pose and range and its public API reads an imported mass, rpm and battery: a measured build would then be simulated end to end on both engines, closing the largest remaining fidelity-scope gap.
\item \textbf{Implement and gate the remaining part commands} (positioning sensor, camera, bumper and gripper) exactly as the ultrasonic, inertial and line-follower commands already are, so every fitted part offers a working, gated command.
...
\item \textbf{A source-cited research mode}, opt-in only, that searches manufacturer datasheets and reputable suppliers for candidate parts, records retrieval dates and regions, and keeps electrical and purchasing decisions visibly unverified until a competent person checks them.
\item \textbf{An asset and interchange import workflow} on top of the shipped specification import: a documented path for authoring parts and terrain, and a loader for URDF descriptions and glTF geometry, so published robot models can be used directly rather than only their numbers.
\item \textbf{A research bridge} to the established heavyweight simulators, exporting a build and program for high-fidelity validation and pulling the results back, which keeps Kodro light while opening a path to rigorous physics.
\item \textbf{A bridge to a robotics middleware}, an optional separate component talking to a running daemon over a local socket so a validated program can drive real topics, recorded as a long way out because it fights the offline guarantee.
\item \textbf{The human pilot after ethics approval} specified in the evaluation, comparing deterministic evidence with the raw console over three diagnosis missions, followed only if justified by studies of classroom learning and refinement-memory efficacy.
\end{enumerate}

\section{Closing statement}
The originality of this work is not a new algorithm but the implemented combination of specification-driven behaviour, disclosed fidelity, a lightweight simulation, build-aware generation and deterministic rejection in one accessible tool. A skeptical builder can compare design choices and export the assumptions behind a result; a learner has a path from a three-step sequence to a marked Key Stage 4 exercise in which every lesson is machine-verified as finishable. Whether that path teaches anybody anything is not established here, and Section~\ref{sec:threats} says so directly. The strongest justified claim is mechanical: Kodro catches defined classes of invalid program, refuses to badge a figure it does not simulate, and holds its own lesson library to a gate rather than an assertion. Physical predictive validity, memory-driven improvement, learning gain and usefulness to real users remain open.
