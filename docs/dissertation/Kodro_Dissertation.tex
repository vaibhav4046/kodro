...
\titleformat{\chapter}[display]
  ...
  % \chaptertitlename, not the literal word: it tracks \appendix, so the
  % appendix headings read "Appendix A" rather than "Chapter A".
...
\hypersetup{
  ...
  % Left empty on purpose: the marked copy is anonymous from the second
  % title page on, and PDF properties travel with every page.
  pdfauthor={},
}

% ---- Anonymous volunteer questionnaire toggle -------------------------------
% ON since 30 August 2026, after the supervisor's reply on the review plan.
% \humanfeedbackfalse removes the questionnaire material and restores the
% no-human-data ethics statements in one line if that ever needs undoing.
% Refresh the counts and the figure from a same-day export before submission.
\newif\ifhumanfeedback
\humanfeedbacktrue
\newcommand{\ethicsroutenote}{The questionnaire and its inclusion in this report were agreed with my supervisor.}

% The module brief counts screenshots, diagrams, tables and code snippets as
% one consecutive sequence of figures, so tables share the figure counter and
% its name, and the code listings carry figure captions.
\makeatletter
...
At commit \texttt{e70b98b}, recorded with its working-tree status in \texttt{docs/\allowbreak eval/\allowbreak test\_\allowbreak suite.json}, the declared Python matrix collects 1,641 tests and passes all of them at 90.85 percent branch-aware coverage against an 85 percent gate, a figure that is a floor rather than an exact value for reasons Chapter~6 states. The interpreter harness passes 180 of 180 checks; four Prove contracts pass all twenty seeded runs, reproduce byte-identically and reject a broken controller; the shipped web build passes its boot, privacy, contrast, responsive, behaviour and world gates in full; and ten further gates totalling 303 checks hold the teaching half of the product, so that every one of the twenty-four lessons ships a worked solution that scores 100 out of 100 through both marking engines. A live-Ollama run and a synthetic-persona artefact complete cleanly, and neither is human-usability evidence. Renderer measurements meet the 4.17 ms work budget of a 240 Hz display on the host's integrated GPU and miss it under forced software rasterisation, the disclosed floor for a machine with no working graphics driver. These are engineering verification results, not evidence that the simulation predicts a physical robot to deployment tolerance or that real users find the product useful. \ifhumanfeedback No human usability study is reported; a short anonymous volunteer questionnaire is reported as feedback only in Section~\ref{sec:volunteerfeedback}.\else No human study is reported.\fi{} The contribution is the integration of specification-driven behaviour, disclosed fidelity, build-aware generation and deterministic rejection into a zero-cost early design and learning tool.

\frontsection{Statement of Ethical Compliance}
\ifhumanfeedback
The evaluation combines simulated personas and automated traces with one short anonymous questionnaire completed by adult volunteers, reported in Section~\ref{sec:volunteerfeedback}. The questionnaire collected no name, email address or any other personal data, required no sign-in, and carried its consent statement on the face of the form; no conclusion about real users in general is drawn from it. \ethicsroutenote{} The synthetic personas remain language-model constructs, labelled as such at every point of use, and none of the evaluation in Chapter~6 draws a conclusion about real users beyond that questionnaire's stated limits. The synthetic and automated evaluation continues to sit in data category A0 with participant category 2. The work followed the module's ethical guidance throughout.
\else
The reported evaluation uses simulated personas and automated traces only. It involves no human participants and no personal data used as research data, and it therefore sits in ethics data category A0 with participant category 2. No ethics application was required for the work reported here, and none of the evaluation in Chapter~6 draws a conclusion about real users. The work followed the module's ethical guidance throughout.
\fi
...
% twenty chapter-level entries costs most of a page of contents listing.
\renewcommand*\l@chapter[2]{%
  ...
  \fi}
\makeatother
% The contents heading uses the same compact style as the other preliminary
% sections; the report class's \chapter* form spends most of a page on drop.
\clearpage
{\setlength{\parskip}{0pt}%
 \par\addvspace{12pt}{\large\bfseries\color{uolblue}\contentsname\par}\nopagebreak\vspace{1pt}%
...
\looseness=-1 The obvious answer is simulation: try the robot in software first, watch it behave, fix what is wrong, and only then build it. This is exactly what professional roboticists do with heavyweight simulators, and the difficulty is that those simulators are heavyweight in every sense. They assume a workstation, a graphics card, an internet connection for assets and accounts, and an operator who already knows robotics: they are built for the expert refining a known design, not for the non-expert exploring whether an idea is worth pursuing at all. A person who has never wired a board cannot sit down in front of an industrial simulator and get useful work done in an afternoon.

\looseness=-1 There is a second answer that has become tempting in the last few years, which is to ask a large language model to write the control code. This works until it does not, and the way it fails is dangerous in this setting. A language model produces plausible text, and plausible control code that calls a sensor the robot does not have, or that assumes a capability the hardware lacks, reads exactly like correct control code to a beginner. The output looks authoritative and is sometimes wrong, and the person least able to tell the difference is the very person the tool is meant to help.

\looseness=-1 There is a third failure that the first two share, and it is the one Kodro is named against. A simulator that reports a tidy number without saying how much to trust it invites the same misplaced confidence as a hallucinating model. A pretty run that quietly runs a slow robot faster than it can really go, or reports a top speed it never actually simulates, is a lie told in the language of engineering. The response is not to hide the approximations but to badge them: to say plainly, figure by figure, which numbers the simulation honours, which it approximates, and which it merely reports without ever driving.

\looseness=-1 Kodro is built where these observations meet. It is a proving ground for reducing uncertainty before purchase: a place where a skeptical builder imports a robot specification and inspects how that design behaves under a disclosed first-order model on one ordinary laptop, with no account or subscription. It treats the language model as a useful but untrusted assistant rather than an oracle, grounds it on the active command set, and places deterministic validation between generated code and the editor. It treats its own physics the same way, disclosing the fidelity of every reported figure rather than presenting an approximation as a certified result, and it serves the learner who has only an idea through a parts catalogue, lessons and a design studio.
...
\looseness=-1 \emph{Offline-first} means that all core design, lesson, simulation and deterministic checking paths work with no account, paid interface or remote service. Local Ollama is optional. The user may explicitly choose a cloud model, which is a connected mode rather than part of the offline guarantee.

\looseness=-1 \emph{Realistic} means visually and behaviourally legible within a stated model, not physically identical to reality. The robot must respond to its build and share the world with moving agents, and a run must report a spread rather than one tidy outcome.

\emph{Honest} means that every figure is labelled honoured, approximated or not simulated.

\looseness=-1 \emph{Safe} means safe to inspect and test in the simulated workflow: generated code is withheld when validation fails. It does not mean that Kodro certifies software for deployment on physical hardware.

\section{Research questions}
The aim above is an engineering aim, and the objectives that follow it are engineering objectives. Four research questions sit underneath them, and these are the questions Chapter~6 is organised to answer and Figure~\ref{fig:closure} answers in one place.
...
\looseness=-1 One question is deliberately absent. This dissertation does not ask whether Kodro teaches anybody anything, because answering that requires learners, and no human study was run. Section~\ref{sec:threats} states what follows from that omission.

\section{Aims and objectives}
The aim is a working offline proving ground on which a builder imports a real robot specification, or a non-expert designs one, and validates the behaviour in an agent-populated simulation before building it, with every reported figure carried at an honestly stated fidelity. Each objective carries a test of done, so progress is judged rather than asserted.

\subsection{Core objectives}
These define the artefact. O1 to O5 are load-bearing, and each is reported as met or not met in Figure~\ref{fig:closure}. O6 was planned as core and is kept at that level, but its test of done is an experiment rather than a property of the code, and that experiment was not run; its status is carried openly below and in Section~\ref{sec:threats} rather than quietly downgraded.

\begin{enumerate}[label=O\arabic*.]
\item \looseness=-1 \textbf{Specification-driven behaviour, imported or designed.} Import a measured robot specification, or assemble one from a parts catalogue, and let it drive the simulation. \emph{Done when} changing the build changes behaviour: a heavier robot accelerates more slowly and drains its battery faster, a stronger motor raises the closed-form top speed, and a fitted sensor unlocks its command while an absent one withholds it everywhere.
\item \looseness=-1 \textbf{Honest fidelity disclosure.} Report every performance figure as honoured, approximated or not simulated, never an approximation as a certified result. \emph{Done when} each figure carries its badge in the Lab, the dashboard and the per-robot report, and a value the simulation does not drive is never badged honoured.
\item \looseness=-1 \textbf{Safe execution and scoring.} Execute the robot's real program in a sandbox that blocks file, network and process access, and score every run against the scenario's success criteria. \emph{Done when} a hostile program cannot reach the disk and a correct one is scored correctly.
\item \looseness=-1 \textbf{Realistic, randomised validation.} Run the robot among terrain and moving agents over repeated randomised runs. \emph{Done when} one program runs across several seeds and reports successes, collisions, time to goal and battery used.
\item \looseness=-1 \textbf{Grounded local assistance with a guaranteed fallback.} Prefer a local model grounded in the specification, falling back to deterministic behaviour when none is present. \emph{Done when} the platform stays usable with the network off and no model installed, and code calling a capability the build lacks is never offered for application.
\item \looseness=-1 \textbf{Refinement from use without retraining.} Retrieve accumulated reflections and reusable skills to inform later suggestions. \emph{Done when}, in an ablation over repeated related tasks, enabling memory lowers the median iterations to a working design. \emph{Status:} storage and retrieval are implemented; the causal criterion is not demonstrated.
\end{enumerate}
...
\item \looseness=-1 \textbf{Two ways to program, one meaning.} Provide a text editor and a blocks palette that compiles to the same program, so that the way the user expresses a behaviour does not change what the behaviour means.
\item \looseness=-1 \textbf{One shared motion model.} Drive the simulation from a single set of closed-form equations and constants, mirrored across the two engines and locked together in continuous integration, so the visible studio and the tested Python engine cannot silently disagree about how a robot moves. \emph{Done when} a constants hash, a golden-trace corpus and a formula-parity test all gate the two engines on every push.
\item \looseness=-1 \textbf{Legible realism on a normal laptop.} Render the worlds, the named mission sites and the robot with enough fidelity to read as believable in a screenshot, while staying smooth on a machine with no discrete graphics card.
\item \looseness=-1 \textbf{An honest record.} Build the system as small, independently tested changes behind a continuous integration gate, and keep a decision log and a test-evidence log, so the work is reproducible and the dissertation can be checked against it.
\end{enumerate}

\subsection{Out of scope}
Two things are deliberately not attempted. Kodro does not aim at production-grade physical accuracy: its motion is a believable kinematic model rather than a rigid-body solver, and Chapter~7 is explicit about what that does and does not buy. It also does not aim to replace Isaac Sim, Gazebo, Webots or MuJoCo; the useful relationship with those tools is the research bridge on the roadmap, not a claim of equivalence.
...
\begin{itemize}
\item \looseness=-1 an offline-by-default application that imports a measured robot specification, or assembles one from a parts catalogue, and derives its mass, closed-form top speed, battery life and exact runnable command set.
\item a three-tier fidelity disclosure, honoured, approximated and not simulated, carried as per-figure badges, a realism dashboard and a per-robot verification report.
\item \looseness=-1 one shared motion model, closed forms and constants mirrored in JavaScript and Python and locked by a constants hash, a golden-trace corpus and a formula-parity test, so the two engines cannot silently drift.
\item \looseness=-1 two ways to program one robot, a Python-subset interpreter and a blocks palette compiling to the same Python, and a build-aware assistant that withholds invalid output after bounded repair, with a deterministic path when no model is installed.
\item a validation layer that runs a program across seventeen mission sites among moving agents, with an experience memory available to later sessions without retraining.
\item KodroBench, a grounding metric and seeded harness measuring by syntax-tree analysis whether a generated program stays inside the build's fitted-command set.
\item a development process under continuous integration with a decision log and a test-evidence log, which is what gives this dissertation a factual basis.
\end{itemize}

\subsection{What is new here}
None of the ingredients is a new algorithm: retrieval grounding, sandboxed execution, experience memory, skill libraries and domain randomisation are each established and none is claimed here. Four things are new in combination, as far as the surveyed work shows, and each is put as a checkable claim rather than a boast.

\begin{enumerate}[label=N\arabic*.,leftmargin=*,itemsep=1pt]
\item \looseness=-1 \textbf{Grounding measured against a per-build interface.} Existing harnesses score generated robot code against one fixed interface, so the hallucination they can catch is a bad argument to a real primitive. Because Kodro derives the command set from the user's own build, the ground truth for a valid call changes per design: the same call is grounded on one robot and invented on the next. KodroBench measures exactly that, and Section~\ref{sec:kodrobench} reports it catching an entire invented command surface in this project's own fine-tuned models.
\item \looseness=-1 \textbf{Fidelity disclosure enforced by construction rather than by prose.} Simulators routinely document their assumptions in a manual. Here the three tiers are attached to each figure at the point of use and held by a gate, so a value the simulation does not actually drive cannot be badged as honoured. Section~\ref{sec:panel} records the gate catching two real violations in this project's own build.
\item \looseness=-1 \textbf{A teaching library held to a machine-checkable standard.} Every lesson's own worked answer is executed through both marking engines on every change and must score full marks within the constructs that lesson has taught, so ``every lesson is finishable'' is a gate rather than an editorial promise (Section~\ref{sec:classroom-gates}).
\item \looseness=-1 \textbf{One specification driving behaviour, capability and marking together.} The specification decides how the robot moves, which commands exist to be written, and therefore what the grader accepts: changing a motor changes top speed and battery life, and removing a part changes the set of programs that compile at all.
\end{enumerate}

What is emphatically not claimed: physical predictive validity, a learning gain, or a causal benefit from the memory store. The mechanism of system memory is demonstrated and its efficacy is not.

\section{Report structure}
The contents page gives the full order. Two conventions are worth stating in advance: Chapters~4 and~5 separate design from implementation but are written to be read together, and Chapter~6 keeps what was measured strictly apart from what is only planned.

\chapter{Background and Related Work}

Three lines of work shape the design; this chapter reads each for what it implies and ends by naming the niche they leave open.

\section{Learning by building}
\looseness=-1 The oldest line is constructionism, the durable finding that people learn most deeply when they build something external that they can watch behave and then debug \citep{papert1980}. The important word is \emph{watch}: a learner who sees the consequence of a change and can change it again is in a tight feedback loop that abstract instruction cannot match, and Kodro is built around that loop, assemble a robot, write a behaviour, run it, watch, adjust. The blocks palette compiles to the same Python the editor runs, so moving from blocks to text changes the surface and not the tool, and nothing understood is thrown away in order to progress.

\section{Trusting a simulator: the gap between simulation and reality}
The second line asks why a simulator is worth trusting at all. The central problem is the gap between simulation and reality, the drop in performance when a behaviour tuned in simulation meets the physical world, and the accepted response is not a perfect simulator but domain randomisation: vary the friction, the mass, the sensor noise and the scene across many runs, so that a behaviour which survives the spread is likely to survive reality too \citep{tobin2017}. Variability is the asset, not the enemy: a program that only works on one tidy layout has learned the layout, and one that works across a randomised spread has learned something more general.

Kodro adopts the principle at a lightweight, pre-build level, exercising a design across randomised seeds with varied friction, mass and sensor noise, among moving agents rather than on an empty plane, and reporting the spread rather than only the average; this is robustness testing within a first-order model, not evidence of transfer to reality.

\section{The model: hallucination, grounding, and honest refinement}
\looseness=-1 The third line concerns the language model itself and justifies both the offline choice and the careful framing of refinement. Two findings shape the design: hallucination is intrinsic to these models rather than a defect a future version will simply remove \citep{huang2025}, and in generated code it surfaces as plausible but wrong calls \citep{lee2025}, precisely the dangerous failure when the output is control code a beginner cannot evaluate. The standard mitigation is to ground generation in retrieved, authoritative material rather than in the model's memory \citep{lewis2020}; for robots, the strongest form is to constrain the model to compose a known, safe set of capabilities rather than invent them, in the spirit of programs written against a fixed robot interface \citep{liang2023,dossantos2026} and of grounding instructions in what the robot can actually do \citep{ahn2022}.

\looseness=-1 Kodro supplies the active command set with each generation request, but prompt grounding alone cannot stop a model inventing a missing sensor call, so the implementation validates the returned syntax tree against that set, attempts bounded repair and withholds the code if validation still fails; the grounding material lives on the machine, so a local model with no cloud suffices. Genuine weight-level self-evolution remains an open problem \citep{tao2024} and Kodro does not claim it: its system memory follows verbal reflection \citep{shinn2023} and reusable behaviour libraries \citep{wang2023}, with recent work offering stronger experience-driven versions \citep{wu2025}, and the existence of these mechanisms does not establish efficacy in Kodro, which requires the planned ablation. Deterministic checks retain final authority because multi-agent arrangements have characteristic failure modes \citep{cemri2025}; automated review may flag suspect code \citep{sun2025}, while deterministic syntax-tree analysis provides the enforceable rejection boundary \citep{khati2026}.

A fourth, narrower body of work makes the local option practical rather than merely principled: locally deployable models paired with retrieval have shown feasibility on institutional hardware \citep{reza2025}, and onboard small models are being investigated for robot task planning and code generation \citep{wang2026}. These results motivate a local path without establishing that every one to four billion parameter model is reliable on Kodro's command surface, so the evaluated models are reported by task outcome and the design relies on validation rather than assumed capability.

\section{The niche}
\label{sec:niche}
\looseness=-1 The reviewed work leaves a narrower integration gap: professional simulators prioritise high-fidelity modelling and expert extensibility, educational environments prioritise accessible programming against supported or preconfigured robots, and within the products reviewed for this project none combined a user-entered physical specification, explicit per-figure fidelity labels, lessons, build-aware code generation and deterministic rejection in an offline-first interface. This is a bounded market observation, not proof that no product anywhere has overlapping features. The nearest evaluation harness in the literature, RoboEval with its CodeBotler programs \citep{hu2023}, checks generated code against one fixed robot interface; KodroBench measures an invented symbol against an interface that changes with the build.

\looseness=-1 Official documentation supports that segmentation rather than any claim that the alternatives are deficient: Gazebo offers high-fidelity physics, rendering and sensor models behind plugin and messaging entry points \citep{gazebo2026}; Webots is a desktop simulator for modelling and validating robots that expects some robotics knowledge \citep{webots2026}; VEXcode VR deliberately gives a preconfigured virtual robot in a browser \citep{vex2026}; and Open Roberta centres drag-and-drop programming of supported boards \citep{openroberta2026}. Appendix~\ref{app:comparison} sets the four beside Kodro on the axes that decide which one a school can actually run. Kodro's differentiator is the workflow it integrates: start with a catalogue or measured specification, expose the resulting command set, test code in a lightweight world, and label what the model does not simulate; optional cloud keys are a connected mode outside the offline guarantee.

A second argument for the offline design is stronger than the usual privacy and cost ones: a hosted educational tool depends on somebody continuing to pay for the hosting, and that dependency has a documented failure mode. Trinket, the embedded Python widget used across many United Kingdom Key Stage 3 and GCSE teaching resources, shut down on 31 August 2026, eleven days before this dissertation was due \citep{trinket2026}. Its announcement is explicit that open-sourcing the code did not preserve the service: the site went dark, every resource that embedded a trinket lost its interactive element, and saved work not exported in time went with it.

The point is not that one company withdrew a service; it is that the architecture decides whether a teaching tool can be withdrawn at all. A tool whose lessons, marking, world and pupil record are files on the machine cannot be switched off by a third party, cannot lose work to an account deletion, and cannot be broken by a change to an interface it never calls. The honest comparison with a hosted competitor is lifetime against lifetime rather than feature against feature, and this project claims only the former.

\chapter{Requirements and Analysis}

\looseness=-1 This chapter turns the aims of Chapter~1 into requirements the design can be measured against. It starts with the intended user, and with who the tool is not for, because that decides most of what follows.

\section{The intended user, and who it is not for}
\label{sec:users}
\looseness=-1 Kodro is built for three related users, and the third arrived late enough to change what the product is. The first is the skeptical early-stage builder comparing a robot idea before committing money and hardware, who imports measured values and reads the fidelity report as a record of model assumptions, separating figures the simulation honours from approximations and omissions. The second is the capable non-expert with an idea but no lab, kit or training, a maker exploring a companion robot or a hobbyist comparing rover designs, who can assemble a catalogue build and test behaviour on the laptop already to hand. Kodro helps both form and reject early hypotheses. It tells neither that the physical design is proven.

\looseness=-1 It is equally important to say who the tool is not for, because a tool that claims everyone as its user serves no one. Kodro is not for a professional roboticist refining a known design to deployment tolerances. That person needs rigid-body physics and the established heavyweight simulators, and Kodro does not pretend to compete with them. It is not a gamified coding toy, and it is not a general-purpose environment for adults who already know robotics.

\looseness=-1 Its third and now largest user is a school pupil working through the lesson path, and that has to be stated plainly because it changed during the project: the lesson library, the marking, the hint and worked-answer sequence and the reading-age handling were all built for that user, and they now constitute most of the product. The twenty-four lessons span Key Stage 1 to Key Stage 4 and remain weighted to the upper end, with three at KS1, four at KS2, nine at KS3 and eight at KS4. The lower end was thickened late, from a single Key Stage 1 taster to a three-lesson beginner floor plus two fault-finding lessons, so "Key Stage 1 to 4" now describes a usable entry path rather than a token presence at each stage; the range is still densest at Key Stages 3 and 4, where the product is strongest.

\looseness=-1 \looseness=-1 Four situations shaped the requirements below, and the product handles each end to end today. A builder \textbf{choosing between two motors} enters both sets of measured numbers and runs one program against each, comparing speed, battery life and whether the robot still clears the obstacle, with no hardware bought. A teacher \textbf{setting a lesson on a locked-down laptop} opens the static web build with no administrator rights, no discrete graphics card and no internet, and gets a mark derived from the run the pupil watched. A hobbyist \textbf{checking a part-built rover} imports its measured mass, wheel radius, motor rpm and sensor mounts, then reads the verification report to see which figures the simulation drives and which it only repeats back. A teacher \textbf{authoring without a cloud account} registers the local server with a coding assistant and has the grader the pupil will face check the lesson's own answer before it ships.

\looseness=-1 It is not for a team collaborating over a network on a shared design: the offline, single-machine constraint that makes Kodro safe and private also makes it solitary. It is not for anyone who needs the model to be brilliant, because the model is small, local and grounded on purpose. Naming these non-users is not modesty; it is the discipline that keeps the rest of the requirements honest.

\section{Functional requirements}
The functional requirements follow the core objectives as testable capabilities, each naming the evidence that would satisfy it rather than the feature implementing it. Eleven describe the platform: import or assemble a robot (F1); the specification drives behaviour, so a heavier build accelerates more slowly and drains battery faster (F2); every performance figure carries a fidelity tier (F3); a command appears only when the part it needs is fitted (F4); text and blocks compile to one command surface (F5); programs run in a sandbox with no file, network or process access (F6); a run is scored against scenario criteria with a per-criterion reason (F7); one program is validated across several randomised seeds (F8); the assistant is grounded on the fitted-command set and its output validated against it (F9); a deterministic rule engine keeps the platform usable with no model installed (F10); and a reflection is recorded after a run and retrieved later (F11). Five more came with the lesson path: every lesson ships a worked answer that scores full marks through both marking engines, so a lesson whose own answer fails does not ship (F12); a mark derives from the run the learner watched, with any re-run disclosed on screen (F13); a stalled learner receives graded hints before the worked answer (F14); an authored lesson cannot be saved until its own answer passes the marker the learner faces (F15); and learner names, programs and attainment are written to the local machine only (F16). Appendix~\ref{app:requirements} states each in full with its test of satisfaction; the chapters that follow cite these identifiers, and Chapter~\ref{ch:evaluation} reports the result of each test.
...
\begin{itemize}
\item \looseness=-1 \textbf{Offline by default.} No account, no paid interface, and no data leaving the machine on any default path: the core's only networked call is to a local model server on loopback, and the platform is fully functional without it. A user may connect their own cloud key, sending only their prompt and only to the provider they chose; the offline default is never weakened.
\item \looseness=-1 \textbf{Runs on a mainstream laptop.} The worlds must render smoothly with no discrete graphics card, which bounds the visuals to procedural geometry and a measured lighting model rather than heavyweight assets.
\item \textbf{Deterministic where it matters.} The simulation, the scoring and the safety checks must be reproducible, testable without a display, and trustworthy to mean the same thing twice.
\item \looseness=-1 \textbf{Safe by default.} An absent model, a missing audio device, a headless environment or a hostile program must each produce a clear, contained outcome rather than a crash or an unguarded action.
\item \looseness=-1 \textbf{Legible.} A non-expert must be able to read the program surface and understand it, which is why the command API is a small set of plainly named functions rather than an object hierarchy.
\end{itemize}

\section{The central analysis: a helpful model against a trustworthy result}
\looseness=-1 The hardest requirement to satisfy is the tension between F9, helpful model assistance, and F6, safe execution: helpfulness pushes toward flexible generation while trust requires rejecting anything beyond the defined surface, and a naive design hands a beginner plausible but wrong control code where an overly rigid one helps nobody. Kodro separates the two by authority. The model may draft, suggest and explain, but never decides that a program is acceptable; deterministic machinery owns that boundary, precisely because handing the decision to a second model would move the failure rather than remove it, multi-agent arrangements having characteristic failure modes of their own \citep{cemri2025}. Automated review may flag suspect code without owning the decision to accept it \citep{sun2025}. Chapter~4 builds the machinery that enforces the split; the principle is that the model proposes and the deterministic layer disposes.
...
The three-stage shell is an input-process-output model made visible to the user. Figure~\ref{tab:ipo} records the contract of each stage and prevents a presentation-only redesign from obscuring what the system actually does.
...
The scorer (Figure~\ref{fig:scorer}) illustrates the trace-driven, pure-core principle from the design. It takes a scenario, a trace and the source, and returns a structured result with no input, no output and no hidden state, which is exactly what makes it testable with synthetic traces.
...
\end{lstlisting}
\captionof{figure}{The scorer's decision core: a pure function from lesson, trace and source to a structured result.}
\label{fig:scorer}

\section{Showcases, icons, and project files}
Three showcase programs ship as example tabs and double as living documentation of the command surface, each verified in the interpreter QA sweep. Encore is the flagship: functions, loops, nested loops, counters and pen geometry over the base command set alone, so it runs on every build. Searchlight is a battery-managed survey and rescue mission that runs a systems check and patrols a search area with every drive guarded by the distance sensor. Gauntlet is a capability reference that draws its own proof, exercising recursion, nested loops, chained comparisons, list building and pen geometry. Around these, a procedural SVG icon set replaces every emoji-as-icon, each a handful of primitives built offline against \texttt{currentColor} so it renders identically on every operating system, which the interpreter QA enforces by asserting zero emoji glyphs remain in the chrome source. A whole project, the build, the programs, the active world, the render settings and the accumulated memory, serialises to and from a single \texttt{.kodro} JSON file with bounded sizes and a validator that never throws, so a session is portable and a backup is a file copy.
...
The privacy position is split, and it is the one boundary on the offline claim. Speaking is local: the layer accepts only voices the browser reports with \texttt{localService} set true, and with no local voice installed it turns speaking off and says why rather than falling back, because the network voices listed in that same collection are served by sending the text away. Listening is not local and cannot be made so in the browsers this targets, since the recognition interface delegates the captured audio to a vendor service, and which vendor is not the product's to know: the desktop build renders inside the platform web view, so the recogniser belongs to whoever wrote that engine. An earlier opt-in that named one company was therefore wrong everywhere that company's engine was not running, and naming a specific wrong recipient is a worse failure of consent than naming none. The wording now states only what holds on every engine, that the recording leaves the machine and that the receiving service belongs to the browser's maker rather than to the platform, and the gate forbids the notice from naming a single vendor at all. Dictation is off by default, reachable only through that opt-in, withdrawable, and the microphone is never left open because recognition is requested one utterance at a time. A pupil who never turns it on sends nothing, and a school that forbids it can leave it off with no other part of the platform degrading. The visual \texttt{say} command is unrelated to any of this and remains what it always was, a console line the robot writes.

One hundred and eight assertions hold the layer, run in Node against its pure functions with no browser present: that a list of only network voices yields no voice at all, that the opt-in notice names no single vendor, that dictation is off until consent is given and off again when withdrawn, that spoken sentences parse identically to typed ones, and that a spoken reply never reads a code block aloud. Eighteen of them pin the verbatim output of a real recogniser rather than a guess, because a measured benchmark contradicted the code: a small offline transcriber run over ten synthesised clips of the product's own commands, recorded in \texttt{docs/eval/stt\_bench.json}, never once returned the product name as spelt, producing \emph{Caudreau}, \emph{Cottro} and \emph{Caudrill}, none of which the shipped list of expected mishearings held. The consequence was not cosmetic: with the name left in front of the sentence, \emph{how do I make the rover go faster} parsed as a command and set the speed, and every spoken \emph{stop} failed to interrupt a reply. A phonetic test on the leading token replaced the rule, and the ten transcripts are pinned in the gate along with the words it must not consume, \emph{quiet} first among them because it is itself an interruption.

Fourteen of those assertions cover the lesson library, which is what the layer must reach to be more than a novelty. A lesson resolves whether named by topic, title or number, and the routing table's ordering is asserted so a specific lesson wins over the generic one sharing a word in its title. Near-misses are held as tightly as hits, because opening a lesson resets the stage and swaps the program buffer, so a question about a lesson, a hypothetical about starting one and a request to write code for one all leave the library where it was. The table duplicates a fact the library already owns, so it is gated both ways: a test fails if it names a lesson that is not shipped, and fails again if a shipped lesson has no phrase that reaches it, which stops a new lesson being reachable by mouse alone.
...
Twelve methods were used, in increasing cost and decreasing frequency. Six are continuous automated gates that run on every push: the Python test suite, the interpreter QA harness, the world and interface nets, the Prove contract replay, the lesson solvability and marking gates, and the learner-record and error-path gates. Four are periodic measured studies: the formative persona review, the deterministic persona-task evaluation, the renderer evidence and the KodroBench grounding benchmark. One is the seven-round adversarial persona panel. The twelfth, a user and refinement study that would measure real efficacy, is planned and has not been run; no figure in this chapter comes from it. Figure~\ref{tab:evalstrategy} in Appendix~\ref{app:evaldata} states what each method checks and its status. The sections below report each in turn.

\subsection*{Where the numbers in this chapter come from}
Almost every figure reported below is emitted by a harness into a versioned artefact under \texttt{docs/eval} rather than transcribed by hand, and each artefact carries a schema version, a generation timestamp and the commit or working-tree state of its run, so a reader can tell which run produced a number and whether the artefact matches the source state it claims. Figure~\ref{tab:provenance} in Appendix~\ref{app:evaldata} lists which harness writes which artefact and what each records. Two exceptions are named there and repeated where they are reported. The nine offline module gates print their totals to standard output and emit no JSON, so their commands and printed lines are transcribed into \texttt{qa\_gate\_runs\_2026-08-18.md}, which says on its face that it is a hand-written transcript and is checked by re-running the command in its first column rather than by trusting the file. The adversarial persona panel of Section~\ref{sec:panel} kept its round-by-round tallies in session notes rather than in a harness artefact, and is the one set of figures in this chapter a reader cannot re-run at all.
...
Nine further offline gates cover module surfaces the browser matrix cannot reach, and were re-run on 18 August 2026. Eight of them pass 335 of 335 checks between them, over the extracted physics module, the interpreter regression pins, the parts catalogue, the memory-graph builder, project opening, the quality-tier classifier, the browser model facade and the honesty sweep that enforces the claim boundaries described above. Section~\ref{app:gates} gives the per-gate counts and what each one asserts. Two results carry qualifications that belong here rather than in an appendix. The model-facade gate includes the checks that rewriting an object-style call into the metre-based API cannot change a distance by a factor of one hundred, and that a lesson left incomplete is never written into memory as a successful program, which are correctness properties rather than counts. And the ninth gate, on grammar constraints, passes six of six and is counted apart from the 335 because two of its checks need a local model server. Those two ran here: a prompt explicitly demanding a command this build does not fit returned a program without it, once, on one model, which is the constraint holding and not a rate.
...
\label{sec:kodrobench}
The methods above evaluate the product. A final method measures, in isolation and with a number, the specific failure the grounding design exists to prevent: a language model emitting a program symbol the built robot does not expose. The nearest neighbour is RoboEval with its CodeBotler harness \citep{hu2023}, which executes language-model programs for service robots and checks temporal properties over the traces; its programs target one fixed interface, so the hallucination it surfaces is an invalid argument to a valid primitive. General interface-hallucination benchmarks measure invented calls but involve no robot and never execute the program. None of the surveyed work measures an invented \emph{symbol} against a \emph{per-build} interface, where a model grounded for one robot can invent for the next and the same call is valid on one build and hallucinated on another. KodroBench measures exactly that intersection.

The metric is deliberately small and deterministic. The grounding checker parses a generated program into its abstract syntax tree and classifies each call. A bare-name call is invented when it names neither a command in the build's fitted-command set nor one of the three builtins the sandbox exposes, unless the program itself defines or aliases that name. An attribute call is invented when it is made on an object the program never created, since the fitted API is a set of bare functions and exposes no objects: \texttt{rover.forward()} is therefore counted, and only a real built-in method on a name bound to a literal container, such as \texttt{xs.append(1)} after \texttt{xs = []}, is exempt. That attribute rule is what lets the metric catch an entire object-style command surface rather than waving it through as ordinary Python. A syntax error is recorded as a result rather than raised, and the function is pure, so the same program and fitted set always return the same verdict.

Around it, the benchmark harness gives each model a natural-language instruction together with exactly the commands the build exposes, then runs the generated program across ten seeded, domain-randomised runs through the same Python engine the desktop application uses, following the randomisation argument of \citet{tobin2017}. A run succeeds when the program moves at least the task's minimum distance without a collision, and the reported success is the mean over seeds and tasks. The harness also supports the unbiased pass@$k$ estimator of \citet{chen2021} when several samples are drawn per task, and carries a development and held-out split whose two held-out tasks are adversarial: the instruction names a capability the build lacks, such as a distance sensor it does not carry. With five tasks this split is a mechanism rather than a rigorous held-out claim, and growing the suite toward twenty to fifty tasks is what would make it meaningful.

The results are written as a machine-readable record and the leaderboard is generated from that record, never typed by hand, so every number in Figure~\ref{tab:kodrobench} traces to a run. A deterministic floor, a fixed grounded program per task, is byte-reproducible and gives continuous integration a value to assert against.
...
The headline finding is honest and not the one that was hoped for. The two models that invent are the project's own local fine-tunes, produced earlier for the assistant role. \texttt{kodro-coder} emits an object-style \texttt{rover.forward()} call surface on every one of the five tasks; against each task's fitted-command set, which for the benchmark's Python engine is a set of bare functions with no \texttt{rover} object, every such call is flagged, giving an invention rate of 1.00 and a task success of 0.00. \texttt{kodro-fast} invents on three tasks of five and drifts into unrelated prose on the other two. The general small models never invent a symbol: given the fitted-command list in the prompt, they stay inside it, and their failures are weaker ones, malformed indentation, missing arguments, or a plausible name used as a bare statement, so their programs mostly parse but rarely complete the task. The zero collision rate of the two fine-tunes is failure rather than caution, since a program built from invented calls never drives the robot at all. The deterministic floor completing only 0.22 says the seeded obstacle field is genuinely hard for a fixed open-loop program, which keeps the model rows in proportion.

Two readings follow, and both matter to the design. First, the metric works: it caught, mechanically and at scale, a failure mode of this project's own artefacts that a compile check or an opinion score would have missed, an entire invented command surface, and the result is reported precisely because it cuts against the project's own fine-tunes. Second, the claim must stay narrower than a blanket vindication. Every invented call in the table is a call to a symbol outside the task's fitted-command set, exactly the class of mistake the metric exists to name, but whether such a call is refused at run time depends on the engine: the web studio's interpreter accepts \texttt{rover.forward()} as a documented compatibility alias for the bare verbs and would run the same program. The benchmark therefore quantifies invention against the per-build fitted set the model was prompted with, on which the object surface is ungrounded, without claiming the product refuses every such call on every surface.
...
\ifhumanfeedback
\section{Anonymous volunteer feedback on the released product}
\label{sec:volunteerfeedback}
% BEFORE SUBMITTING THE ON VARIANT: refresh every count in this section from a
% same-day CSV export of the form, keep that export, and delete this comment.
After the release described above, a short anonymous questionnaire was circulated to adult volunteers known to the author. The form collected no name, email address or account, required no sign-in, restricted respondents to adults, and stated on its face that anonymous summaries may appear in a university project report. It asked five multiple-choice questions covering respondent background, clarity of purpose, first success at running a program, usefulness of lesson feedback and willingness to recommend, plus one optional free-text question. Respondents were asked to spend ten to fifteen minutes with the released web build first, to run a program and, if they could, to try one lesson; the consent text and the six questions as issued are reproduced in Section~\ref{app:questionnaire}.

This is a favourable convenience sample of people known to the author, reported as anonymous volunteer feedback on the released product, not as a usability study: it does not replace the planned human evaluation above and no general claim is drawn from it. At the time of writing six volunteers had responded: three from computing or engineering, one from another field, and two who selected none of the offered backgrounds. Three found the purpose clear straight away and three after a little looking. Four ran a program easily, one after some figuring out, and one got stuck. On lesson feedback, one respondent said it told them exactly what to fix, four that it pointed them in the right direction, and one found it present but confusing. Four would definitely recommend the product to a beginner or a teacher and two probably would. Of the three free-text answers, one asked for more course content and requested no fixes, one said the interface could be more user friendly, and one answered a different question than the one asked and is counted as a non-response, a small instrument lesson recorded for any future questionnaire. The most informative pattern is that the only respondent who got stuck and the only one who found the feedback confusing sit among the two non-technical volunteers, which is the closest signal in this data to the product's intended audience and points at the first-run experience. Figure~\ref{fig:volunteersummary} in Appendix~\ref{app:evaldata} reproduces the form's response summary, built from the live form's own charts and verbatim free-text answers at the moment of capture. No change was made to the product in response to this feedback before submission. With more time, the change it points at is the first run for a non-technical adult: a guided first minute that puts a program on screen before the free canvas does, re-tested with the same questionnaire.
\fi

\section{Summary of the evaluation}
The product is strongly verified against its stated software contracts, but usefulness and physical predictive validity are not established. The 1,641-test Python matrix, 180-check interpreter harness and deterministic web gates pass at the commit recorded in \texttt{docs/eval/test\_suite.json}. The four Prove contracts pass twenty seeded runs, replay byte-identically and reject the broken controller. The local-Ollama checks run cleanly, but their generated phrasings are not people. The renderer report preserves a negative result: its software-rendered host missed the 240 Hz work budget in both tiers. The adversarial synthetic panel found real defects, but its outputs remain bug-finding evidence rather than user evidence.\ifhumanfeedback{} Six adult volunteers gave anonymous feedback on the released product, favourable overall, with the two non-technical respondents supplying the only stuck run and the only confusing-feedback rating; it is feedback, not a study.\fi{} The planned human study is ethics-pending and has not been run. No claim of memory-driven improvement, classroom efficacy, purchasing accuracy, universal frame rate or physical transfer follows.
...
\chapter{Professional Issues and Reflection}
This chapter states the ethical position, shows the six outcomes the module asks for, and closes with a first-person reflection on the project.

\section{Ethics}
\ifhumanfeedback
The only human input reported in this dissertation is the anonymous volunteer questionnaire of Section~\ref{sec:volunteerfeedback}, which collected no name, email address or any other personal data and required no sign-in; respondents were adults and the form carried its consent statement on its face. \ethicsroutenote{} No other reported work involved human participants. The evaluation personas are language-model constructs, not people, and every section heading, table caption and summary sentence that reports a persona figure states this at the point of use, so a reader who meets one of those tables out of context cannot mistake it for user data. The synthetic and automated evaluation continues to sit in data category A0 with participant category 2.
\else
...
The module brief asks that six outcomes be shown, and each is met by material this document already carries.
\begin{enumerate}[label=(\arabic*),leftmargin=*,itemsep=1pt]
\item \textbf{A systematic understanding and critical awareness of current problems.} Chapter~2 reads the constructionist, simulation-to-reality and model-grounding literature for what each implies for the design, and Section~\ref{sec:threats} states what the evidence cannot support.
\item \textbf{A comprehensive understanding of applicable techniques.} Chapters~4 and~5 apply retrieval grounding, syntax-tree validation, sandboxing, domain randomisation, golden-trace conformance and coverage-gated testing to one artefact.
\item \textbf{Originality in the application of knowledge.} Section~\ref{sec:contributions} states the contribution as an implemented combination rather than a new algorithm, and Section~\ref{sec:kodrobench} measures an axis the surveyed prior art leaves unoccupied.
\item \textbf{Complex issues handled systematically and creatively, with sound judgement under incomplete data, communicated to specialist and non-specialist readers.} The fidelity tiers, the Prove contracts and the classroom gates each turn a judgement into a checkable rule, and Chapter~7 states the limits in plain language.
\item \textbf{Self-direction and originality in tackling problems.} The project changed scope toward the lesson path (Section~\ref{sec:users}), removed and rebuilt the voice route (Section~\ref{sec:voice}), and was planned, gated and released under version control and continuous integration (Section~\ref{sec:engineering}).
\item \textbf{Critical self-evaluation of the process.} Section~\ref{sec:reflection} below, with Section~\ref{sec:threats} and Chapter~7.
\end{enumerate}

\section{Self-reflection}
\label{sec:reflection}
This section is written in the first person, as the module brief invites, and it keeps to what the record supports.

What worked was building scoring, hinting and replay on one trace abstraction and keeping the decision cores pure, which paid off repeatedly in testability and in adding features late.

What was harder than expected was headless interface testing, where a browser launch can time out on a loaded machine and the failure names the wrong culprit: an \texttt{ETIMEDOUT} on process spawn says nothing about the page, so a gate that fires it is measuring the host, not the product. The temptation was to demote those nets to a smoke signal. Timing one flow end to end settled it instead: on the development laptop a flow took 2 minutes 56 seconds against a 60 second ceiling and still produced a valid screenshot, which made the ceiling the defect rather than the product. That observation lives in the gate source rather than in a separate artefact, since the timing was a debugging measurement rather than a controlled experiment. Making the ceiling platform-aware, and overridable by environment variable, returned the browser nets to hard gates that pass in full, and the lesson generalised: a flaky gate is a measurement problem until it has been measured, and the deterministic interpreter QA and Python suite are load-bearing because they are fast and reproducible, not because the browser evidence is untrustworthy.

The reflection that carries the most weight for a proving ground is that honesty is not a property you assert once but one you have to enforce and repair: a panel review of the version 2.0 build found two defects in the exact fidelity surfaces the product stakes its credibility on, a slow robot shown faster than real under an honoured badge and a predictive check that contradicted the live simulation, and only fixing both, and collapsing four drifted kinematic replicas onto one gated shared model, made the fidelity claim true rather than aspirational. What I would do differently is to wire the feedback loop and the shared motion model first rather than last, because both were the foundation the rest of the honesty rests on, and discovering their absence through review rather than by design cost rework.

The result I am least comfortable with, and most glad I reported, is the KodroBench row for my own fine-tunes (Section~\ref{sec:kodrobench}). I spent real time producing \texttt{kodro-coder} and \texttt{kodro-fast} for the assistant role, and the benchmark shows one of them inventing an entire command surface on every task. The temptation was to leave the row out, or to score the fine-tunes only on the studio surface that happens to accept their output. Reporting it as measured cost nothing but pride, and it turned the benchmark from a demonstration into an instrument: a metric that never catches its author's own artefacts has not been shown to catch anything.

A second lesson arrived after the evaluated commit, so its figures are not in Chapter~6. I ran a mutation-testing pass over the four modules that decide a pupil's outcome, the grader, the motion model, the sandbox and the executor, behind a suite that was green at over ninety percent branch coverage. It found a real bypass: the sandbox's constant folder ignored a boolean where it read the integer one, so \texttt{True * 10 ** 9} laundered a bignum bomb past the guard that \texttt{1 * 10 ** 9} could not pass. It also showed that the correctness of the thread-kill path rested on a single comparison no test exercised. Both are fixed and pinned by regression tests in the code archive. The lesson is that a passing test proves only that the code does what its author expected; I should have written tests that try to break the decision modules from the start, not tests that confirm them.

The project changed identity in its second half. The lesson path grew from a single taster into the largest part of the product (Section~\ref{sec:users}), and I did not re-plan when that happened. The cost is visible: objective O6 was planned as core and its ablation was never run, and the human study stayed at the protocol stage while the effort went into twenty-four lessons and the gates that hold them. I would make the same trade again, because the lessons gave the product a real audience, but I would make it explicitly and early, and I would settle the ethics route for a human evaluation in the first month rather than the last, so that it could have been designed rather than deferred.

The voice route was a dead end I entered twice (Section~\ref{sec:voice}). The first build treated speech as a third authoring route and was cut because it could not be made reliable under the offline rule; the rebuild authors nothing, and even then its opt-in notice had to be corrected when I found that naming one vendor as the recipient of the audio was wrong on every engine that vendor did not run. The principle that a recording leaving the machine must be disclosed exactly was already written into the design; I had not applied it to a feature I wanted to ship.

Feedback on my earlier proposal document was that its prose read as machine-generated, that it lacked a BCS section and that ethics was not treated separately. I took all three seriously. This dissertation was rewritten in a plainer voice, with the generative tools declared on the Declaration page used under my direction for drafting, revision and checking, their outputs treated as proposals rather than evidence; and ethics and professional issues have this chapter to themselves. If I were advising a student starting this project, I would say that the honesty machinery, the fidelity badges, the provenance record and the gates, is not overhead to add at the end. It is the part of the product a skeptical reader can verify, and it is the part I am proudest of.

\chapter{Conclusion and Future Work}

\section{Summary}
\looseness=-1 This project set out to build a free, offline-first proving ground in which a builder imports a robot specification, or a non-expert designs one, programs it through text or blocks and tests the design in an agent-populated simulation before buying hardware. It delivers that, behind a default three-stage Design, Prove and Build journey with an opt-in Expert surface. At the commit recorded in \texttt{docs/eval/test\_suite.json} the 1,641 Python tests, 180 interpreter checks, twenty seeded Prove runs and the web gates all pass, an intentionally broken controller fails, and objective O6 remains incomplete because no controlled evaluation shows that memory reduces iterations. Figure~\ref{fig:closure} closes every objective and research question from Chapter~1 against the section carrying its evidence.

\looseness=-1 One sentence needs to stay narrower than it wants to be. The simulation demonstrably rejects programs a build cannot run, marks a run against the world that was on screen, and reports a spread rather than one tidy figure. Whether that reduces a real builder's uncertainty is a claim about a person, and no person was measured. The platform does not reproduce or certify a physical robot, and it has not been shown to change what a builder decides.

\begin{table}[h]
\centering\footnotesize
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{@{}p{0.9cm}p{2.5cm}p{11.6cm}@{}}
\toprule
Item & Outcome & Evidence \\
\midrule
O1 & Met & The specification drives motion, battery and command availability (Sections~\ref{sec:spec}, \ref{sec:shared-motion}, \ref{sec:autoverify}). \\
O2 & Met, after repair & Fidelity tiers by construction (Section~\ref{sec:fidelity}) with a per-robot report (Section~\ref{sec:verreport}); two badge defects the panel found were fixed (Section~\ref{sec:panel}) and the honesty gate holds the boundary. \\
O3 & Met & Sandbox, executor and gate (Section~\ref{sec:safety}), verified by the suite and the interpreter harness (Section~\ref{sec:autoverify}). \\
O4 & Met & Prove contracts replayed over seeds with varied conditions, each run's metrics and verdict recorded (Section~\ref{sec:autoverify}). \\
O5 & Met & Grounded local assistant with a deterministic fallback (Section~\ref{sec:assistant}); residual invention risk measured, not assumed (Section~\ref{sec:kodrobench}). \\
O6 & Not met & The memory mechanism is built; the efficacy ablation was not run (Sections~\ref{sec:threats} and~\ref{sec:memory-limit}). \\
O7 & Met & Blocks compile to the same Python the editor runs (Section~\ref{sec:interp}); the Blocks path is in the browser matrix (Section~\ref{sec:autoverify}). \\
O8 & Met & A constants hash, golden traces and formula parity gate both engines on every push (Section~\ref{sec:conformance}). \\
O9 & Met on the host only & Renderer budget met on the integrated GPU, missed on the software floor (Sections~\ref{sec:renderer}, \ref{sec:framerate}). \\
O10 & Met & Decision log, evidence log, continuous integration (Section~\ref{sec:engineering}) and artefact provenance. \\
RQ1 & Yes & One motion model mirrored in two engines and held together by automated gates rather than inspection (Section~\ref{sec:conformance}). \\
RQ2 & Yes, engine-scoped & Syntax-tree validation flagged every invented symbol prompt grounding let through, offline (Section~\ref{sec:kodrobench}); run-time refusal depends on the engine. \\
RQ3 & Yes, by construction and gate & No figure is badged honoured unless the simulation drives it (Section~\ref{sec:fidelity}); the gate caught and closed two violations (Section~\ref{sec:panel}). \\
RQ4 & Yes & Every lesson's worked answer must score full marks through both markers on every change (Section~\ref{sec:classroom-gates}); agreement is not validity. \\
\bottomrule
\end{tabular}
\caption{Closing the brackets opened in Chapter~1: each objective and research question with its outcome and where the evidence sits.}
\label{fig:closure}
\end{table}

\section{Future work}
The roadmap follows from the limitations, in roughly the order the work should be taken on.

\begin{enumerate}
\item \looseness=-1 \textbf{Wire the imported specification through the Python engine}, so its sensor rays honour an imported sensor's mount pose and range and its public API reads an imported mass, rpm and battery: a measured build would then be simulated end to end on both engines, closing the largest remaining fidelity-scope gap.
\item \looseness=-1 \textbf{Implement and gate the remaining part commands} (positioning sensor, camera, bumper and gripper) exactly as the ultrasonic, inertial and line-follower commands already are, so every fitted part offers a working, gated command.
\item \textbf{Rigid-body physics on the hot path}, making the tested physics wrapper the source of truth for visible motion, so the viewport reflects contact forces, friction and collision response.
...
\item \textbf{Richer domain randomisation} over obstacle placement, lighting, friction and sensor noise, so that a passing program is one that generalises rather than one that memorised a layout.
\item \looseness=-1 \textbf{A source-cited research mode}, opt-in only, that searches manufacturer datasheets and reputable suppliers for candidate parts, records retrieval dates and regions, and keeps electrical and purchasing decisions visibly unverified until a competent person checks them.
\item \looseness=-1 \textbf{An asset and interchange import workflow} on top of the shipped specification import: a documented path for authoring parts and terrain, and a loader for URDF descriptions and glTF geometry, so published robot models can be used directly rather than only their numbers.
\item \looseness=-1 \textbf{A research bridge} to the established heavyweight simulators, exporting a build and program for high-fidelity validation and pulling the results back, which keeps Kodro light while opening a path to rigorous physics.
\item \looseness=-1 \textbf{A bridge to a robotics middleware}, an optional separate component talking to a running daemon over a local socket so a validated program can drive real topics, recorded as a long way out because it fights the offline guarantee.
\item \looseness=-1 \textbf{Deployment and scale without raising the hardware floor.} The static build is already a plain folder that a school can serve from a file share or a single-board computer with no account and no connection; the open questions are an optional local-network aggregation service, run by the school on its own hardware and off by default, so a teacher can see a class without giving up the offline guarantee, and keeping the present kinematic model as a selectable tier if rigid-body physics ever reaches the hot path. Appendix~\ref{app:comparison} sets this platform's requirements against the published minimums of four others and works the three directions through.
\item \looseness=-1 \textbf{The human pilot after ethics approval} specified in the evaluation, comparing deterministic evidence with the raw console over three diagnosis missions, followed only if justified by studies of classroom learning and refinement-memory efficacy.
\end{enumerate}

\section{Closing statement}
\looseness=-1 The originality of this work is not a new algorithm but the implemented combination of specification-driven behaviour, disclosed fidelity, a lightweight simulation, build-aware generation and deterministic rejection in one accessible tool. A skeptical builder can compare design choices and export the assumptions behind a result; a learner has a path from a three-step sequence to a marked Key Stage 4 exercise in which every lesson is machine-verified as finishable, though whether that path teaches anybody anything is not established here. The strongest justified claim is mechanical: Kodro catches defined classes of invalid program, refuses to badge a figure it does not simulate, and holds its own lesson library to a gate rather than an assertion. Physical predictive validity, memory-driven improvement, learning gain and usefulness to real users all remain open.
...
% The list grew with the platform-requirements sources; tightening the
% inter-entry space keeps it on the sheets it already had.
...
\bibitem[Cyberbotics(2026)]{webots2026} Cyberbotics (2026) \textit{Webots user guide: foreword}. Available at: \url{https://www.cyberbotics.com/doc/guide/foreword} (Accessed: 17 July 2026).

\bibitem[Cyberbotics(2026b)]{webotsreq2026} Cyberbotics (2026b) \textit{Webots user guide: system requirements}. Available at: \url{https://cyberbotics.com/doc/guide/system-requirements} (Accessed: 9 September 2026).
...
\bibitem[Gazebo(2026b)]{gazeboreq2026} Gazebo (2026b) \textit{Troubleshooting: rendering}. Available at: \url{https://gazebosim.org/docs/latest/troubleshooting/} (Accessed: 9 September 2026).
...
\bibitem[NVIDIA(2026)]{isaacsim2026} NVIDIA (2026) \textit{Isaac Sim: system requirements}. Available at: \url{https://docs.isaacsim.omniverse.nvidia.com/latest/installation/requirements.html} (Accessed: 9 September 2026).
...
\bibitem[Open Roberta(2026b)]{openrobertalocal2026} Open Roberta (2026b) \textit{Lokale Installation}. Fraunhofer IAIS. Available at: \url{https://www.open-roberta.org/lokale-installation/} (Accessed: 9 September 2026).
...
\bibitem[VEX Robotics(2026b)]{vexvr2026} VEX Robotics (2026b) \textit{VEXcode VR}. Available at: \url{https://www.vexrobotics.com/vexcode/vr} (Accessed: 9 September 2026).
...
% Appendix chapters are listed; their sections are reached by \ref from the body.
...
\end{lstlisting}
\captionof{figure}{The command surface as presented to the user, with the parts each gated command needs.}
...
for step in range(14):
    if distance() < 130:
        ...
        move_forward(1)
\end{lstlisting}
\captionof{figure}{The first example program, loaded in the editor on first launch.}
...
\label{app:evaldata}
Chapter~\ref{ch:evaluation} states the result of each method and the limits that qualify it. This appendix holds the artefact record behind the chapter, the strategy overview, the underlying breakdowns for three of the methods and the volunteer questionnaire as issued. Nothing here is new: every figure below is reported in the body at the point the method is discussed.

\begin{table}[h]
\centering\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{2.8cm}p{4.9cm}p{7.2cm}@{}}
\toprule
Method & Artefact under \texttt{docs/eval} & What it records \\
\midrule
Python matrix and coverage & \texttt{test\_suite.json} & collected and passed counts, the coverage figure, and the commit and working-tree status of the run \\
Prove contracts & \texttt{prove\_baseline.json}\newline \texttt{prove\_report.md} & the contract set, the engine, the seed root, the declared evidence boundary and the overall verdict \\
Renderer measurements & \texttt{performance\_eval.json}\newline \texttt{performance\_eval\_\allowbreak hardware.json} & one file per graphics mode: the method, the browser and platform, the 240\,Hz frame-time target and hashes of the artefacts measured \\
Interface gates & \texttt{ui\_eval.json} & the same pattern; a narrowed \texttt{--suite=} run writes its own per-suite file, deliberately kept out of version control so a partial run cannot be mistaken for the full one \\
Live-Ollama run & \texttt{vibe\_eval.json} & the model, the endpoint, per-measure results and totals \\
Synthetic-persona tasks & \texttt{persona\_eval\_\allowbreak results.json} & the model, the base seed, the turn limit and the cell count \\
Nine offline module gates & \texttt{qa\_gate\_runs\_\allowbreak 2026-08-18.md}\newline \texttt{qa\_gate\_runs\_\allowbreak 2026-08-14.md} before it & a hand-written transcript of each command and its printed totals; the gates emit no JSON, so the check is to re-run the command \\
Adversarial persona panel & none & round-by-round tallies kept in session notes; not re-runnable \\
\bottomrule
\end{tabular}
\caption{Where the figures in Chapter~\ref{ch:evaluation} come from. Every JSON artefact carries a schema version and a generation timestamp. The last two rows are the two exceptions named in Section~\ref{sec:evalstrategy}.}
...
\begin{figure}[p]
\centering
\includegraphics[height=0.92\textheight,keepaspectratio]{img/form_summary.png}
\caption{Response summary of the anonymous volunteer questionnaire at N=6, taken from the live form: the five multiple-choice distributions and all three free-text answers, including the one counted as a non-response. Discussed in Section~\ref{sec:volunteerfeedback}.}
\label{fig:volunteersummary}
\end{figure}
\fi
...
\label{app:gates}
Section~\ref{sec:autoverify} states the aggregate for these gates and the two results that carry qualifications. This section gives the counts behind that aggregate. All were re-run on 18 August 2026, at commit \texttt{1887b07} with a clean working tree, and seven of the nine reproduced their 14 August figures exactly.

The extracted physics module passes 25 of 25 collision, ray, coverage and velocity checks under Node with no browser and no React in the loop. The interpreter regression gate pins nine named hardening fixes and passes 13 of 13. The parts catalogue passes 40 of 40 provenance, unit-conversion and schema-range checks, and the memory-graph builder passes 22 of 22 derivability, determinism and sparse-input checks. Project opening passes 16 of 16 checks that a partial write cannot be reported as success, and the quality-tier classifier passes 46 of 46 checks driven as a pure function without a browser. The browser model facade passes 51 of 51 checks on the pure string logic that turns a local model's raw text into runnable code. The honesty sweep that enforces the claim boundaries stated in Chapter~6 passes 122 of 122, one more than on 14 August because the gate gains assertions as the claim surface grows. Those eight sum to the 335 quoted in the body.

The ninth, the grammar-constraint gate, passes six of six and is counted apart from that total because two of its checks need a local model server. Its offline four drive the schema and the compiler as pure functions: that the fitted command set reaches the schema's enumeration intact, that the compiler emits the calls it should, that it drops out-of-set and invented commands, and that it formats a text argument correctly. Its live two ran for the first time in a recorded session, against a local model on the development host. The prompt they send explicitly demands \texttt{distance()}, a sensor read this build does not fit, and the two assertions are that the returned program contains no call outside the fitted set and that \texttt{distance()} in particular does not appear. It did not, on three consecutive runs. Two assertions against one model on one prompt is not a sample, so this is evidence that the constraint held on that occasion and carries no rate and no claim about local models in general. The model name and the printed output are recorded in \texttt{docs/eval/qa\_gate\_runs\_2026-08-18.md} so that the boundary is visible from the artefact and not only from this paragraph.

\ifhumanfeedback
\section{The volunteer questionnaire as issued}
\label{app:questionnaire}
The questionnaire, titled ``Kodro: a five-minute anonymous review'', was a hosted web form with email collection switched off, no sign-in and no name, email or identifier question. Its description doubled as the consent text and is reproduced here with the link to the released build removed for anonymity.

\begin{quote}\small
Kodro is an offline robot design and simulation studio for learning to code, built as an MSc project. Please spend 10 to 15 minutes with it first: open it, click ``Free play'' or ``Learn to code'', run a program, and try one lesson if you can. Link: [the released web build]. This review is anonymous and voluntary. No email address, name or account is collected, and you can stop at any time by closing the tab. You must be 18 or older to respond. Anonymous summaries of the responses may be used in a university project report. Submitting the form means you are happy with that.
\end{quote}

\begin{enumerate}[label=Q\arabic*.,leftmargin=*,itemsep=1pt]\small
\item Which best describes you? \emph{Student or working in computing / engineering; Student or working in another field; Teacher or educator; None of these.}
\item After your first few minutes, how clear was it what Kodro is for and where to start? \emph{Very clear, I knew what to do straight away; Mostly clear, I found my way after a little looking; Somewhat unclear, I had to guess; Not clear, I did not know what to do.}
\item Did you manage to run a program and watch the robot carry it out? \emph{Yes, easily; Yes, but it took some figuring out; No, I got stuck; I did not try this.}
\item If you tried a lesson, how useful was the feedback when something went wrong? \emph{It told me exactly what to fix; It pointed me in the right direction; It was there but confusing; I did not try a lesson.}
\item Would you recommend Kodro to a beginner learning to code, or to a teacher? \emph{Definitely; Probably; Probably not; Definitely not.}
\item (optional, free text) What is the one thing you would change, and the one thing you would definitely keep?
\end{enumerate}
Questions 1 to 5 were required and single-choice. Section~\ref{sec:volunteerfeedback} reports the aggregated answers and Figure~\ref{fig:volunteersummary} reproduces the form's own summary.
\fi

\chapter{Platform Comparison and Hardware Requirements}
\label{app:comparison}
Section~\ref{sec:niche} argues that the reviewed platforms leave a specific gap rather than that they are deficient. This appendix supports that with the vendors' own published requirements, because hardware is what decides which of these a given school or hobbyist can actually run. Every figure below is quoted from the supplier's documentation on 9 September 2026 and is cited to it; where a supplier publishes no figure the cell says so rather than guessing, and \emph{not published} should be read as an absence of evidence, not as an absence of a requirement.

\begin{table}[h]
\centering\footnotesize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}p{2.05cm}p{3.0cm}p{1.5cm}p{1.65cm}p{1.6cm}p{3.5cm}@{}}
\toprule
Platform & Graphics & System RAM & Account & Internet & Cost and licence \\
\midrule
\textbf{Kodro} & no dedicated card; measured on integrated graphics, degrades to software rasterisation & not specified & none & none & free, MIT \\
Gazebo & OpenGL 3.3+, no vendor named; software rendering officially supported \citep{gazeboreq2026} & not published & not published & not published & free, Apache 2.0 \\
Webots & dedicated NVIDIA or AMD card, OpenGL 3.3, 512\,MB video memory; Intel graphics discouraged \citep{webotsreq2026} & 2\,GB & not published & not published & free, Apache 2.0 \\
Isaac Sim & GeForce RTX 4080 minimum, 16\,GB video memory; cards without RT cores unsupported \citep{isaacsim2026} & 32\,GB min, 64\,GB rec. & not stated plainly & required, for the asset service \citep{isaacsim2026} & free for internal R\&D; source Apache 2.0 \\
VEXcode VR & browser, graphics acceleration needed for correct sensor data \citep{vexvr2026} & not published & none on the free tier & required; the paid offline client revalidates at least every 30 days \citep{vexvr2026} & free tier limited to blocks; enhanced tier priced per educator per year \\
Open Roberta & not published & not published & not required \citep{openrobertalocal2026} & not required if self-hosted & free, Apache 2.0 \\
\bottomrule
\end{tabular}
\caption{Published minimum requirements, from each supplier's own documentation, accessed 9 September 2026. Self-hosting Open Roberta is officially supported on a Raspberry Pi 2 or later with 1\,GB of memory \citep{openrobertalocal2026}. MuJoCo publishes no minimum memory or graphics requirement and states only that it needs a processor with AVX instructions.}
\label{fig:platformcompare}
\end{table}

Three things follow, and the third is the one that matters to this project.

First, the fidelity ranking is the reverse of the accessibility ranking, and this project is on the accessible end of it. Isaac Sim, Gazebo and Webots model contact forces, sensor physics and rendering that Kodro does not attempt, and their requirements are the honest price of that work. Nothing in this table should be read as a criticism of them: a professional refining a design to deployment tolerances needs what they provide and should use them, which is why Section~\ref{sec:users} names that person as a non-user of Kodro and why the roadmap proposes exporting to those tools rather than competing with them.

Second, the two axes that decide classroom viability are not fidelity at all. They are whether the machine can render the scene and whether the room needs an account and a connection. On the first, Webots asks for a dedicated card and discourages the integrated Intel graphics that school hardware most often has, and Isaac Sim's floor is a recent gaming or workstation card. On the second, Isaac Sim documents an internet requirement outright, and VEXcode VR's free tier is browser-hosted with the offline client behind a paid tier that still revalidates on a schedule. Open Roberta is the closest to Kodro on both counts: it requires no account and is officially self-hostable, including on a Raspberry Pi.

Third, and this is the narrow claim the project actually makes, no platform in this set combines the low floor with a user-entered physical specification that drives the simulation, per-figure fidelity disclosure, and a deterministic gate on generated code. Open Roberta matches the accessibility and programs supported boards rather than a specification the user measures. VEXcode VR deliberately fixes the robot. Gazebo and Webots accept a described robot in full detail and ask for the hardware and the expertise to match. Kodro is not better than any of these at what they are for. It occupies the position where a measured specification, an honest fidelity label and a machine with no graphics card have to hold at the same time, and that is the whole of the claim.

\section{Growing the platform without raising the floor}
\label{app:scaling}
The zero-cost constraint bought a low hardware floor, and that floor is the platform's main asset when a school asks what it needs. Kodro's measured target is a laptop with no discrete graphics card, on which the renderer met the 4.17\,ms work budget of a 240\,Hz display in every sample; the disclosed lower bound is forced software rasterisation, where it misses that budget and the quality tier degrades rather than the application failing. Appendix~\ref{app:comparison} sets those requirements against the published minimums of four other platforms. The short version is that the two capable physics simulators both want a dedicated graphics card, that Isaac Sim additionally specifies a recent RTX card, 16\,GB of video memory, 32\,GB of system memory and an internet connection for its asset service \citep{isaacsim2026}, and that this is entirely reasonable for the work those tools do and entirely out of reach of a school laptop trolley.

\looseness=-1 Three directions would extend that reach rather than raise the floor. \textbf{Deployment} is already the cheapest of them: the static build is a plain folder of files with no server and no Python, so a school can serve it from a file share, a memory stick or a single-board computer on the local network, and nothing about that path needs an account or a connection. \textbf{Scale within a classroom} is bounded by the fact that every record is local, which is the property that makes the tool private and also the property that stops a teacher seeing thirty machines at once; an optional local-network aggregation service, run by the school on its own hardware and off by default, is the honest way to close that without giving up the offline guarantee. \textbf{Scale in fidelity} is where hardware would have to rise: putting rigid-body physics on the hot path, as Section~\ref{sec:framerate} and the roadmap above both anticipate, moves the cost from a kinematic update to a contact solver, and a build that did that would need to keep the current model as a selectable tier so the low-end machine does not lose the product when the high-end machine gains a feature.

The assistant scales the same way and is capped by the same reasoning. The local path targets small models in the one to four billion parameter range through a local server, which is what makes it viable on a machine with no accelerator at all; a larger local model gives better drafts and needs correspondingly more memory, and a user who wants that can supply their own cloud key in the connected mode. Neither route changes the deterministic gate, because the gate is what decides whether generated code is offered, and its cost does not grow with the size of the model behind it.
