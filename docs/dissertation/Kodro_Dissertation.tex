...
The primary evidence of correctness is the Python suite, which grew with the codebase and gated every change. At the candidate release state, the complete declared matrix, including the optional RL and URDF surfaces, passes 1,087 tests with zero skips and 88.21 percent branch-aware coverage against an 85 percent gate. Unit, property-based and headless integration tests pin the procedural API, deterministic physics and scoring, sandbox rejection, hard timeout, shared-motion conformance and the run, score, hint and persistence loop. The separate interpreter harness passes 180 of 180 checks. Strict lint, format and type checks also pass, and the wheel builds successfully.
...
\midrule
Low & 18.7 & 62.7 ms & 51.4 ms & Not met \\
High & 18.4 & 70.4 ms & 37.0 ms & Not met \\
\bottomrule
\end{tabular}
