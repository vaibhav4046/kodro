...
\midrule
Hardware, Low & 122.3 (116.4 to 124.0) & 8.1 ms & 2.8 ms & Met \\
Hardware, High & 145.0 (143.5 to 145.8) & 7.5 ms & 2.8 ms & Met \\
Software, Low & 35.1 (33.7 to 36.8) & 30.5 ms & 5.3 ms & Not met \\
Software, High & 28.6 (20.2 to 29.9) & 40.1 ms & 6.4 ms & Not met \\
\bottomrule
\end{tabular}
\caption{Renderer evidence from Windows headless Chrome, median of three samples per tier with the observed range in brackets. Hardware rows use the host's integrated Intel GPU through ANGLE; software rows force SwiftShader. On ordinary integrated graphics the render submission work meets the 4.17 ms budget a 240 Hz display would require in every sample; under software rasterisation it does not, and that floor is what the adaptive quality governor exists for. Neither row is a claim about displayed FPS, which the display and browser cap.}
\label{tab:renderer}
\end{table}
