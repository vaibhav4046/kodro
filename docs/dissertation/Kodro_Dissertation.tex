...
% and 11 tables, all numbered and referenced by number at the point of use, so a
...
One hundred and four assertions hold the layer, run in Node against its pure functions with no browser present: that a voice list containing only network voices yields no voice at all, that the opt-in notice names both the audio and the recipient, that dictation is off until consent is given and goes off again when it is withdrawn, that spoken sentences parse identically to typed ones, and that the spoken form of a reply never reads a code block aloud. Eighteen of those assertions pin the verbatim output of a local recogniser rather than a guess at it, and they exist because a measured benchmark contradicted the code. A small offline transcriber was run over ten synthesised clips of the product's own five commands, recorded in \texttt{docs/eval/stt\_bench.json}; it never once returned the product name as spelt, producing \emph{Caudreau}, \emph{Cottro} and \emph{Caudrill} instead, none of which the shipped list of expected mishearings held. The consequence was not cosmetic. With the name left in front of the sentence, \emph{how do I make the rover go faster} parsed as a command and set the speed, and every spoken \emph{stop} failed to interrupt a reply. The rule was replaced by a phonetic test on the leading token and the ten transcripts are now pinned in the gate, along with the words the rule must not consume, \emph{quiet} first among them because it is itself an interruption.
...
Software, Low & 22.2 (22.0 to 22.5) & 61.7 ms & 11.7 ms & Not met \\
Software, High & 21.1 (21.0 to 21.3) & 54.6 ms & 8.8 ms & Not met \\
\bottomrule
\end{tabular}
