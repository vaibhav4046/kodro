...
I confirm that this dissertation is my own work and that it has not been submitted, in whole or in part, for any other degree or qualification. Where I have drawn on the work or ideas of others, the source is acknowledged in the text and listed in the references. The software described here was built over the project period and is held in a version controlled repository. Product claims in this edition were checked against the repository state tagged \texttt{v2.0-submission}. Every figure quoted here names the harness that produced it. Most of those harnesses write a committed artefact under \texttt{docs/eval} or \texttt{results} that records its own generation time, so the figure is checked by re-running the named command and comparing against that artefact rather than against this text. A small group of offline gates print their totals to standard output and emit no artefact; their commands and printed lines are transcribed into \texttt{docs/eval/qa\_gate\_runs\_2026-08-14.md}, which states on its face that it is a hand-written transcript, and those figures are checked by re-running the command alone. Four artefacts postdate the tag because their harnesses were re-run after it, namely the synthetic-persona evaluation, the software-rasterised renderer run, the local-model generation run and the full Python test suite; each carries its generation timestamp inside the committed file, and the tables that quote them say which run they come from. The test-suite artefact additionally records the commit it was taken on and the test files added since the tag, because the suite grew after tagging and its headline count therefore belongs to the final source state rather than to \texttt{v2.0-submission}.
...
At the final source state, recorded with its commit and working-tree status in \texttt{docs/eval/test\_suite.json}, the declared Python matrix collects 1,489 tests and passes 1,488 at 90.97 percent branch-aware coverage against an 85 percent gate; the single non-pass is a host-dependent Tk skip, and the coverage figure is a floor because the local run discards the contribution of node-subprocess tests that continuous integration counts. The interpreter harness passes 180 of 180 checks. Four Prove contracts pass all twenty seeded runs, reproduce byte-identically and reject a broken controller. The shipped web build passes five boot and privacy checks, 61 contrast and responsive checks, a browser matrix of six rendered flows, 41 behaviours, six layouts and 13 modal surfaces, and a world sweep of 61 checks. Ten further gates hold the teaching half of the product, totalling 303 checks: every one of the twenty-four lessons ships a worked solution that must score 100 out of 100 through \emph{both} marking engines on every change, within the constructs that lesson declares and its own line budget, so a lesson whose own answer fails does not ship. A live-Ollama run compiles and runs eight of eight prompts through the real interpreter against a 60 percent floor, and the synthetic-persona artefact completes its 40 cells; neither is human-usability evidence. Renderer measurements meet the 4.17 ms render-work budget a 240 Hz display would require in every sample on the host's integrated GPU (median 144.5 FPS Low, 128.2 High) and miss it under forced software rasterisation (median 18.7 FPS Low, 17.1 High), the disclosed floor for a machine with no working graphics driver. These are engineering verification results, not evidence that the simulation predicts a physical robot to deployment tolerance or that real users find the product useful. No human study is reported. The contribution is the integration of specification-driven behaviour, disclosed fidelity, build-aware generation and deterministic rejection into a zero-cost early design and learning tool.
...
The privacy position is split, and it is set out plainly here because it is the one boundary on the offline claim. Speaking is local: the layer accepts only voices the browser reports with \texttt{localService} set true, and on a machine with no local voice installed it turns speaking off and says why rather than falling back, because the network voices a browser lists in that same collection are served by sending the text away to be spoken. Listening is not local and cannot be made local in the browsers this targets, since the recognition interface delegates the captured audio to a vendor service. Dictation is therefore off by default and reachable only through an explicit opt-in whose wording names both the audio and its recipient before anything is enabled, it is withdrawable, and the microphone is never left open because recognition is requested one utterance at a time. A pupil who never turns it on sends nothing, and a school that forbids it can leave it off with no other part of the platform degrading. The visual \texttt{say} command is unrelated to any of this and remains what it always was, a console line the robot writes.

Forty-seven assertions hold the layer, run in Node against its pure functions with no browser present: that a voice list containing only network voices yields no voice at all, that the opt-in notice names both the audio and the recipient, that dictation is off until consent is given and goes off again when it is withdrawn, that spoken sentences parse identically to typed ones, and that the spoken form of a reply never reads a code block aloud.
...
The primary evidence of correctness is the Python suite, which grew with the codebase and gated every change. At the final source state, the complete declared matrix, including the optional RL and URDF surfaces, collects 1,489 tests at 90.97 percent branch-aware coverage against an 85 percent gate, and passes 1,488 of them. The one remaining test is a skip rather than a failure: it is Tk-dependent and the recording host has an incomplete Tcl installation, which is a property of the host rather than of the suite. Those figures are not transcribed. They are generated from the run's own coverage report and JUnit output into \texttt{docs/eval/test\_suite.json}, which also records the commit, the working-tree status and the test files that were added after the \texttt{v2.0-submission} tag, so the count belongs to the final state and not to the tag. The percentage is a floor rather than an exact figure, because the harness drops the coverage contribution of node-subprocess tests on this host while still running them and still counting them in the test total; continuous integration, which applies no such exclusion, measures the same suite at or above the quoted number. Unit, property-based and headless integration tests pin the procedural API, deterministic physics and scoring, sandbox rejection, hard timeout, shared-motion conformance and the run, score, hint and persistence loop. The separate interpreter harness passes 180 of 180 checks. Strict lint, format and type checks also pass, and the wheel builds successfully.
...
Nine further offline gates cover module surfaces the browser matrix cannot reach, and were re-run at the final source state. The extracted physics module passes 25 of 25 collision, ray, coverage and velocity checks under Node with no browser and no React in the loop. The interpreter regression gate pins nine named hardening fixes and passes 13 of 13. The parts catalogue passes 40 of 40 provenance, unit-conversion and schema-range checks, and the memory-graph builder passes 22 of 22 derivability, determinism and sparse-input checks. Project opening passes 16 of 16 checks that a partial write cannot be reported as success, and the quality-tier classifier passes 46 of 46 checks driven as a pure function without a browser. The browser model facade passes 51 of 51 checks on the pure string logic that turns a local model's raw text into runnable code, including that rewriting an object-style call into the metre-based API cannot change a distance by a factor of one hundred, and that a lesson left incomplete is never written into memory as a successful program. The honesty sweep that enforces the claim boundaries described above passes 121 of 121. The grammar-constraint gate passes four of four offline checks; its live half needs a local model server, which was not running for this measurement, so no live figure is reported.
...
\bottomrule
\end{tabular}
\caption{Persona review trajectory across rounds. \textbf{Every persona in this table is a language-model construct scored by the author, not a person, and no row is user evidence.} The figures are reported as measured by the analyst-run method, with the threats to validity stated below. This was a formative method run before the evaluation harnesses were committed: the per-persona responses behind these means were not retained as a machine-readable artefact, so unlike every other table in this chapter these rows cannot be re-derived from the repository and should be read as a trend record rather than as reproducible data.}
\label{tab:persona}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Task completion broken down by persona and by task. \textbf{Each persona is a language-model construct; a 10/10 row means ten generated requests were satisfied, not that ten learners succeeded.}}
...
The lesson path became the largest part of the product, so it carries its own gate set rather than relying on the general suite. Ten harnesses run against the shipped bundle and the on-device stores, and together they assert 303 checks. Table~\ref{tab:classroom} records the run captured for this chapter; every row was executed rather than transcribed, and each harness prints its own totals.
...
\bottomrule
\end{tabular}
\caption{The ten classroom harnesses and their assertion counts. All 303 checks passed on the recorded run.}
\label{tab:classroom}
\end{table}
...
\bibitem[BCS(2022)]{bcscode} BCS, The Chartered Institute for IT (2022) \textit{BCS Code of Conduct for BCS Members}. Available at: \url{https://www.bcs.org/membership-and-registrations/become-a-member/bcs-code-of-conduct/} \textbf{[VERIFY VERSION, URL AND ACCESS DATE BEFORE SUBMISSION]}
