...
\caption{The default Simple proving surface. The four-stage journey, one dominant Run action, editable program and three-dimensional city remain visible; secondary actions and the evidence rail are available on demand.}
\label{fig:city}
\end{figure}
...
Browser evidence is split so that a degraded launcher cannot be mistaken for a product result. The shipped static build passes five of five boot, mount and zero-external-request checks, and the deterministic stylesheet gate passes 61 contrast and responsive assertions over ten themes. A controlled in-application inspection of the evaluated build opened the Studio, focused Learn recommendation, Robot Lab and Build surfaces. At 1280 by 720 and a true 390 by 844 CSS viewport, the measured document width matched the viewport, no horizontal overflow occurred and the browser error log was empty. The wider headless flow and world matrix had passed on the preceding baseline, but its current rerun did not return within ten minutes on this host after Chrome accumulated degraded helper processes. It is therefore recorded as unavailable for this source state, not carried forward as a pass and not attributed to a product defect without a reproducing page failure.
...
\midrule
Low & 19.5 & 56.0 ms & 7.6 ms & Not met \\
High & 18.6 & 64.6 ms & 8.6 ms & Not met \\
\bottomrule
\end{tabular}
