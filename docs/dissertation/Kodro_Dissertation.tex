...
Low-vision (voice-first) & 1/5 (20\%) & Obstacle stop & 0/4 (0\%) \\
...
The reading is candid and it cuts both ways. The assistant reliably produces syntactically valid code, ninety percent compiled, that mostly runs, eighty percent, which is the floor a beginner needs in order not to be stranded on a syntax error. But task-correct behaviour is limited: only thirty percent of cells completed the task; the precise maker phrasing succeeded at sixty percent against twenty percent for the vaguer voices; and the simple counted loop succeeded for every persona while the square and the sensor-gated stop never completed within three turns. This is the honest ceiling of a small model in the one-to-four-billion-parameter range running on a laptop with no cloud, and it is reported as such rather than hidden.

The result that matters for the design is the third row. Of the sixteen programs that ran, fourteen would have driven the robot into the arena wall, almost always because the model over-shot the distance, a plain ``drive forward a few metres'' becoming a thirty-metre move. The deterministic self-test caught every one of these before it reached the learner and returned an actionable correction rather than a crash. The evaluation therefore validates the central architectural choice directly: the small offline model is not trustworthy on its own, which is exactly why Kodro pairs it with a deterministic execution check instead of surfacing raw generated code. The weakness of the model and the value of the safety net are one finding, not two.
