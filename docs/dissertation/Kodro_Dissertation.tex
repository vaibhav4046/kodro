...
\vfill
{\footnotesize\itshape Approximate word count: 20,005. This excludes the title page, contents, references and appendices.\par}
\end{titlepage}
...
The 2.1 candidate was evaluated as a software artefact. On the local Python 3.13 host, 1,239 tests passed and 140 Tcl/Tk cases were skipped because that installation had no working Tk runtime; this result is not substituted for the repository's release coverage gate, which is enforced under Python 3.12 with Xvfb in continuous integration. The interpreter harness passed 180 checks. Nineteen deterministic JavaScript gates passed, including 55 lesson-grader checks, 121 honesty assertions and 79 Lesson Studio checks. All 24 shipped worked answers scored 100 out of 100 through both marking engines, and repository tests require the teacher scheme, curriculum map and answer key to cover the same lesson set. The rebuilt static application passed five of five browser boot and privacy checks, including a mounted studio, a clean console and no application-originated external request. Both Windows executables were built and hashed. These results establish implementation and internal-consistency properties. They do not establish learning efficacy, physical predictive validity, universal accessibility or safety. No human study is reported. The contribution is the implemented combination of specification-driven behaviour, disclosed fidelity, build-aware generation and deterministic rejection in a zero-cost early design and learning tool.
...
\item a validation layer that runs a program in a sandboxed world of seventeen named mission sites among moving agents and reports what happened, together with 24 graded lessons from Key Stage 1 to Key Stage 4 and an experience memory available to later sessions without retraining any model.
...
It is equally important to say who the tool is not for, because a tool that claims everyone as its user serves no one. Kodro is not for a professional roboticist refining a known design to deployment tolerances. That person needs rigid-body physics and the established heavyweight simulators, and Kodro does not pretend to compete with them. It is not a replacement for a teacher, nor a general-purpose environment for adults who already know robotics. One primary user is a school pupil working through the lesson path. The lesson library, marking, hint and worked-answer sequence and reading-age handling were built for that user and now constitute a substantial product surface. The 24 lessons cover Key Stage 1 to Key Stage 4: three at KS1, four at KS2, nine at KS3 and eight at KS4. This distribution is stated because the broad label alone would hide where most of the material sits. Kodro is also not for a team that needs networked collaboration on a shared design. The offline, single-machine constraint that improves privacy also makes the product deliberately solitary. Finally, it is not for anyone who needs an infallible model. The assistant is small, local and advisory; deterministic code owns execution and grading. Naming these non-users is the discipline that keeps the requirements honest.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{img/simple_design.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\linewidth]{img/simple_studio.png}
...
Adversarial persona panel & Reproducible code defects across simulated makers, verified against source & Done, formative panel plus 14-round release audit \\
...
The primary evidence of correctness is the Python suite, which grew with the codebase and gates every change. At candidate commit \texttt{0559257b17ee2b3899bdffa0455c49c984050640}, a workspace-locked local Python 3.13 run passed 1,239 tests and skipped 140 Tcl/Tk GUI cases because that interpreter had no working Tk runtime. The resulting 66 percent local coverage is not release evidence: skipped interface modules remain unexecuted. The release authority is the Linux Python 3.12 Xvfb job, which executes the complete suite and enforces 85 percent branch-aware coverage; Windows and macOS provide cross-platform checks without substituting their headless skips for that threshold. Unit, property-based and integration tests pin the procedural API, deterministic physics and scoring, sandbox rejection, hard timeout, shared-motion conformance and the run, score, hint and persistence loop. Ruff lint, Ruff format and strict type checking pass locally. Both Windows executables build from the candidate; packaging for other platforms remains the responsibility of the tag-triggered release workflow.
...
Browser evidence is split so that a degraded launcher cannot be mistaken for a product result. The candidate static site was rebuilt explicitly before testing and contains the current 24-lesson bundle. It passes five of five boot and privacy checks: the application mounts, recognisable content renders, the studio mounts with a clean console, and the default application makes no external request. The deterministic stylesheet gate passes 61 assertions over ten themes. The wider browser harness covers rendered flows, behaviour, layouts down to 320 CSS pixels and modal surfaces. On the local Windows host, however, Chrome process creation timed out before all six paint assertions, so that attempt is recorded as neither a pass nor a product failure. The Linux candidate CI job is authoritative for the full paint, behaviour, layout and modal matrix. This separation prevents a host launch failure from being reported as product evidence.

The learning surface has its own stronger consistency contract. The grader gate passes 55 checks and runs every one of the 24 shipped worked answers through both marking engines, requiring 100 out of 100 within the lesson's declared constructs and line budget. The Lesson Studio gate passes 79 checks over the authored-document format, validation and storage paths. Seven repository tests also require the scheme of work, curriculum mapping and answer key to name all 24 lessons, and require every published answer to equal the machine-verified YAML solution. A lesson or teacher document can therefore fail the build for disagreeing with the software.
...
That each pass on a freshly fixed build kept surfacing real defects is the clearest evidence that this method is not a rubber stamp, and it is also a plain reminder that a passing test suite is not the same thing as a sound product. Across the seven formative rounds, one hundred and ninety four simulated personas found eighty five confirmed defects, of which seventy five were fixed, eight were deferred with stated reasons, and two were reclassified or found already correct. A later release-focused audit used a separate 14-round ledger and a refutation step for every proposed issue. That ledger contains 56 accepted defects, all marked fixed at the 2.1 candidate, including wrong-source range calculations, missing accessible names, narrow-screen overflow and physically misleading motor advice. It did not record the pre-declared target of two consecutive zero-finding rounds. The defensible conclusion is therefore narrower than ``flawless'': adversarial inspection repeatedly found and closed reproducible defects, but did not prove that none remain.

The honest limits are the same ones that apply to every simulated method in this chapter. These are language-model personas and not people; the panels measure code and interface claims, not learning or satisfaction. A clean round would show only that a particular set of lenses found nothing reproducible. This is still useful because it is a cheap, repeatable net that catches defects before a participant has to encounter them, but it cannot be read as validation and does not license a perfect score. The human study remains the required next step for claims about users.
...
The product is strongly verified against its stated software contracts, but usefulness and physical predictive validity are not established. At the 2.1 candidate, 1,239 Python tests pass locally with 140 disclosed host skips, the 180-check interpreter harness and all 19 deterministic JavaScript gates pass, all 24 lesson answers pass both graders, and the rebuilt static site passes five of five boot and privacy checks. Release coverage and the full Linux browser matrix remain CI-authoritative. The four Prove contracts pass twenty seeded runs, replay byte-identically and reject the broken controller. The local-model checks and synthetic panels provide executable or bug-finding evidence, not evidence about people. The renderer report preserves its historical software-rasterised negative result. The planned human study is ethics-pending and has not been run. No claim of memory-driven improvement, classroom efficacy, purchasing accuracy, universal frame rate or physical transfer follows.
...
2.1.6 & Legal, social and ethical issues & Safety language, privacy boundaries, accessibility gates, study consent and transparent tool disclosure are explicit. \\
2.1.7 & Commercial and economic context & The zero-cost, offline architecture removes per-seat fees and reduces exposure to hosted-service withdrawal, while purchasing advice stays provisional. \\
...
The principal professional safety risk is false confidence. Kodro therefore labels model fidelity, refuses invalid generated commands and states that simulation is not certification. Local-first storage and no default telemetry reduce disclosure, but an operator still has data-protection duties for identifiable classroom records. A configured cloud provider receives prompts only after explicit selection. Accessibility is gated through contrast, keyboard, touch-target, reduced-motion and narrow-viewport checks, with participant work still needed for issues automation cannot observe. Vendored and third-party work is identified. Low-quality rendering and effects downgrades reduce computation, but no environmental benefit is quantified.

\chapter{Conclusion and Future Work}

\section{Summary}
This project set out to build a free, offline-first proving ground in which a builder imports a robot specification, or a non-expert designs one, programs it through text or blocks and tests the design in an agent-populated simulation before buying hardware. The artefact delivers specification-driven behaviour, three-tier fidelity disclosure, mirrored and conformance-gated closed forms, sandboxed execution and scoring, 17 named mission sites, 24 graded lessons, build-aware model assistance and deterministic rejection. The final interaction design defaults to a Simple three-stage Design, Prove and Build journey, retains an opt-in Expert surface and moves secondary tools behind one disclosure control. At candidate commit \texttt{0559257b17ee2b3899bdffa0455c49c984050640}, 1,239 Python tests pass locally with 140 disclosed host skips, the interpreter passes 180 checks, all 19 deterministic JavaScript gates pass, all 24 worked lesson answers pass both graders, and the rebuilt site passes five browser boot and privacy checks. Twenty seeded Prove runs pass and an intentionally broken controller fails. Historical local-model and renderer runs remain bounded engineering evidence rather than user or hardware-generalisation evidence. The reflection and skill stores work as memory mechanisms, but objective O6 remains incomplete because no controlled evaluation shows that memory reduces iterations. The simulation reduces early design uncertainty. It does not reproduce or certify a physical robot.

\section{Reflection}
Four reflections stand out. First, building scoring, hinting and replay on one trace abstraction paid off in testability and made later features cheaper to add. Second, browser verification is an engineering system in its own right. Cold Chrome launches and software rendering can fail before an assertion runs, so the evidence must distinguish a host failure from a product failure and retain a deterministic core underneath the visual net. Third, honesty is not a label added at the end. The adversarial audits repeatedly found places where the interface contradicted the model, including wrong-source range figures, an unscoped Mars verdict and motor advice that initially made the score worse. Each required a source-level fix and a regression gate. Fourth, the shared motion model and evidence ledger should have existed from the first iteration. Adding them late worked, but it caused avoidable rework and made earlier test totals harder to compare. If restarting the project, I would establish those two sources of truth before adding breadth.
...
\bibitem[BCS(2020)]{bcs2020} BCS, The Chartered Institute for IT (2020) \textit{Guidelines on course accreditation}. Available at: \url{https://www.bcs.org/media/11ofljxo/course-accreditation-guidelines.pdf} (Accessed: 13 August 2026).
...
\bibitem[University of Liverpool(2026)]{uol2026ai} University of Liverpool (2026) \textit{Guidance on generative AI in learning, teaching and assessment}. Available at: \url{https://www.liverpool.ac.uk/about/the-university/reports-policies-and-governance/ai-at-liverpool/policies-and-guidance/guidance-learning-teaching-and-assessment/} (Accessed: 13 August 2026).
