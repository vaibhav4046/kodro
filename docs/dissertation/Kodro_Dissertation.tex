...
\looseness=-1 The point is not that one company withdrew a free tier. It is that the architecture decides whether a teaching tool can be withdrawn at all. A tool whose lessons, marking, world and pupil record live in files on the machine cannot be switched off by a third party, cannot lose a pupil's work to an account deletion, and cannot be broken by a change to an interface it does not call. That property is the reason the zero-cost constraint is worth accepting. The honest comparison with a hosted competitor is therefore lifetime against lifetime, not feature against feature, and this project claims only the former.
...
\section{Reflection}
Four reflections stand out. What worked was building scoring, hinting and replay on one trace abstraction and keeping the decision cores pure, which paid off repeatedly in testability and in adding features late.
...
\label{app:evaldata}
Chapter~\ref{ch:evaluation} states the result of each method and the limits that qualify it. This appendix holds the artefact record behind the chapter, the strategy overview and the underlying breakdowns for three of the methods. Nothing here is new: every figure below is reported in the body at the point the method is discussed.
