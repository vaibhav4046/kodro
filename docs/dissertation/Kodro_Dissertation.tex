...
At the evaluated source state, the Python suite passes 1,078 tests at 89.22 percent coverage against an 85 percent gate, and the interpreter harness passes 180 of 180 checks. The complete interface rerun passes six of six flows, 39 of 39 behaviour assertions and 12 of 12 modal checks, including a 320 pixel classroom layout. The world matrix completed 60 checks. One further Nepal check was not executed because the local Chrome process failed to spawn within its timeout, an environment failure that is not counted as a product pass. Live Ollama trials produced validated code for an available build and refused a distance-sensor request when that capability was absent. These are engineering verification results, not evidence that the simulation predicts a physical robot to deployment tolerance or that real users find the product useful. No human study is reported. The contribution is the integration of specification-driven behaviour, disclosed fidelity, build-aware generation and deterministic rejection into a zero-cost early design and learning tool.
...
Two headless browser nets cover the interactive surface, which the deterministic suites do not reach. The complete interface rerun passes six of six flows, 39 of 39 behaviour assertions and 12 of 12 modal checks, including the 320 pixel classroom view and the memory graph. The world matrix completed 60 checks across base worlds, quality tiers, named sites and weather or time combinations. A further focused Nepal case was not executed because Chrome failed to spawn within the timeout, so it is recorded as an environment failure rather than converted into a product pass. These nets remain smoke evidence because browser launch reliability is outside the artefact.
...
The product is strongly verified against its stated software contracts, but usefulness and physical predictive validity are not established. The interpreter and Python gates pass, the interface rerun passes all executed flows, behaviours and modal checks, and the world matrix records one unexecuted environment case rather than hiding it. The historical persona-task run completed 35 percent of cells and showed why deterministic validation is necessary. The adversarial synthetic panel found real defects, but its outputs are bug-finding evidence rather than user evidence. KodroBench likewise exposes ungrounded symbols in the project's own model artefacts. Live tests on the evaluated build add one narrow result: Ollama generated a valid drive, turn and beep program for an available command set and the same path rejected a distance-sensor request against a build without that capability. No claim of memory-driven improvement, classroom efficacy, purchasing accuracy or transfer to a physical robot follows from these results.
...
\section{Summary}
This project set out to build a free, offline-first proving ground in which a builder imports a robot specification, or a non-expert designs one, programs it through text or blocks and tests the design in an agent-populated simulation before buying hardware. The artefact delivers specification-driven behaviour, three-tier fidelity disclosure, mirrored and conformance-gated closed forms, sandboxed execution and scoring, named mission sites, a functional classroom path, build-aware model assistance and deterministic rejection. At the evaluated source state the Python and interpreter gates pass, all executed interface assertions pass, and live Ollama tests demonstrate one valid generation and one missing-capability refusal. The reflection and skill stores work as memory mechanisms, but objective O6 remains incomplete because no controlled evaluation shows that memory reduces iterations. The simulation reduces early design uncertainty. It does not reproduce or certify a physical robot.
...
\bibitem[Huang et al.(2023)]{huang2023} Huang, L., Yu, W., Ma, W. et al. (2023) A survey on hallucination in large language models. \textit{ACM Transactions on Information Systems} (arXiv:2311.05232). DOI: \url{https://doi.org/10.48550/arXiv.2311.05232}
...
\bibitem[Liang et al.(2023)]{liang2023} Liang, J., Huang, W., Xia, F. et al. (2023) Code as policies: language model programs for embodied control. \textit{IEEE International Conference on Robotics and Automation}. DOI: \url{https://doi.org/10.48550/arXiv.2209.07753}
...
\bibitem[Shinn et al.(2023)]{shinn2023} Shinn, N., Cassano, F., Berman, E. et al. (2023) Reflexion: language agents with verbal reinforcement learning. \textit{Advances in Neural Information Processing Systems}, 36 (arXiv:2303.11366). DOI: \url{https://doi.org/10.48550/arXiv.2303.11366}
...
\bibitem[Tobin et al.(2017)]{tobin2017} Tobin, J., Fong, R., Ray, A. et al. (2017) Domain randomization for transferring deep neural networks from simulation to the real world. \textit{IEEE/RSJ International Conference on Intelligent Robots and Systems}, pp. 23 to 30. DOI: \url{https://doi.org/10.48550/arXiv.1703.06907}
