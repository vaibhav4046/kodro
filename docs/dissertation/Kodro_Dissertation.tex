...
At the evaluated source state, tagged \texttt{v2.0-submission} in the repository, the complete declared Python matrix passes 1,201 tests and 87.88 percent branch-aware coverage against an 85 percent gate, with two tests skipped on hosts where Tk is unavailable; the interpreter harness passes 180 of 180 checks. Four Prove contracts pass all twenty seeded runs, reproduce byte-identically and reject a broken controller. The shipped web build passes five of five boot and privacy checks, 61 contrast and responsive checks, and the split browser matrix of six rendered flows, 33 behaviours, six layouts and 12 modal surfaces. A broader world sweep passes 61 of 61 combinations and identity checks. A focused live-Ollama check compiles and runs eight of eight prompts through the real interpreter, while the larger synthetic-persona artefact completes its 40 cells. Neither is human-usability evidence. Software-rendered measurements, taken as three samples per tier, give a median 35.1 FPS in Low and 28.6 FPS in High, and every sample misses the 4.17 ms work budget required for 240 Hz, so no universal frame-rate claim is made. These are engineering verification results, not evidence that the simulation predicts a physical robot to deployment tolerance or that real users find the product useful. No human study is reported. The contribution is the integration of specification-driven behaviour, disclosed fidelity, build-aware generation and deterministic rejection into a zero-cost early design and learning tool.
...
The primary evidence of correctness is the Python suite, which grew with the codebase and gated every change. At the state tagged \texttt{v2.0-submission}, the complete declared matrix, including the optional RL and URDF surfaces, passes 1,201 tests and 87.88 percent branch-aware coverage against an 85 percent gate. Two tests skip on hosts without Tk, which is a property of the host rather than of the suite. Unit, property-based and headless integration tests pin the procedural API, deterministic physics and scoring, sandbox rejection, hard timeout, shared-motion conformance and the run, score, hint and persistence loop. The separate interpreter harness passes 180 of 180 checks. Strict lint, format and type checks also pass, and the wheel builds successfully.
...
The evaluated build publishes a rolling 120-frame report from the real three-dimensional render loop. It separates request-animation-frame cadence from CPU-side render submission because the former is capped by the browser and virtual display while the latter is the work budget the application controls. Table~\ref{tab:renderer} is the committed run from \texttt{scripts/qa\_performance.mjs}; both the harness and the shipped bundle are SHA-256 locked in the result artefact. The harness samples each tier three times (\texttt{--repeat=3}) and records the per-tier minimum, median and maximum, because a single frame-rate reading on a shared machine is not repeatable: the same build and the same harness produced 18.4 and 29.9 FPS in the High tier depending only on what else the host was doing. Reporting one number would have implied a precision the measurement does not have.
...
\midrule
Low & 35.1 (33.7 to 36.8) & 30.5 ms & 5.3 ms & Not met \\
High & 28.6 (20.2 to 29.9) & 40.1 ms & 6.4 ms & Not met \\
\bottomrule
\end{tabular}
\caption{Renderer evidence from Windows headless Chrome using SwiftShader software rendering, reported as the median of three independent samples per tier with the observed range in brackets. The figures are an environment-specific negative result, not a hardware-GPU benchmark or a claim about displayed FPS. The spread is the point: an earlier single-sample reading of this same build gave 18.7 and 18.4 FPS while the host was under load, so a lone figure describes the machine's occupancy as much as the software.}
\label{tab:renderer}
\end{table}
