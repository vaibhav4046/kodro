...
% \humanfeedbacktrue includes the volunteer-questionnaire material and swaps the
% ethics statements to their questionnaire-aware variants. It stays FALSE until
% the supervisor has named the ethics route; the \ethicsroutenote marker prints
% in red in the ON build so an unconfirmed route cannot reach submission
% unnoticed. Refresh the counts in the volunteer-feedback section from a
% same-day export of the form before submitting the ON variant.
\newif\ifhumanfeedback
\humanfeedbackfalse
\newcommand{\ethicsroutenote}{\textbf{\textcolor{red}{[ETHICS ROUTE TO BE CONFIRMED WITH THE SUPERVISOR BEFORE SUBMISSION]}}}
