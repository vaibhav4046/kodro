...
const modalRules = css.match(/\.modal\.lesson-hub-modal\s*\{([^}]+)\}/)[1];
assert.match(modalRules, /grid-template-rows:\s*auto auto auto minmax\(0,1fr\) auto/, 'The five modal rows must reserve scrolling space for lesson content, not the view selector');
