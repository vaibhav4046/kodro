...
Renders ``src/kodro/assets/web/index.html`` in a desktop window
...
from kodro.engine.rover import BATTERY_PER_METRE, Rover
...
from kodro.memory.achievements import PupilContext, check_and_unlock
from kodro.memory.hint_engine import HintContext
from kodro.memory.hint_learning import best_hint
from kodro.memory.pupil_model import (
    class_heatmap,
    suggest_next_lesson,
    update_on_submission,
)
from kodro.memory.store import DEFAULT_DB_PATH, Store
from kodro.paths import app_dir
from kodro.runtime.binding import (
    active_engine,
    binding_lock,
    set_active_rover,
    set_active_world,
...
from kodro.runtime.session import snapshot, world_from_lesson
from kodro.runtime.tracer import Tracer, set_active, set_state_provider

LOG = logging.getLogger("kodro.web")
...
class BridgeAPI:
    ...
    def __init__(self, *, store: Store, lessons: list[Lesson]) -> None:
        ...
        # User-chosen model override (persisted to ~/.kodro/ai_models.json),
    ...
    def export_report(self) -> dict[str, Any]:
        ...
        from kodro.memory.report import export_progress_report
    ...
    _BUILD_SYSTEM = (
        ...
        "Arduino C, which maps onto the same ideas they learned in Kodro "
        ...
        '"maps":[{"kodro":"move_forward(1)","hardware":"how to do it on this board"}],'
        '"total":number}\n'
    ...
    def _fallback_build(budget: float) -> dict[str, Any]:
        ...
        if budget < 20:
            return {
                ...
                "maps": [
                    {"kodro": "move_forward(1)", "hardware": "Pulse the motor pin briefly."},
                    {
                        "kodro": "turn_right(90)",
                        "hardware": "Stop one side so the rover veers.",
                    },
                    {"kodro": "wait(1)", "hardware": "sleep one second in the program."},
                ],
                "total": 5.0,
            }
        return {
            ...
            "maps": [
                {"kodro": "move_forward(1)", "hardware": "Set both motor pins high briefly."},
                {"kodro": "read_distance()", "hardware": "Trigger the HC-SR04, time the echo."},
                {"kodro": "obstacle_ahead()", "hardware": "True when echo distance is small."},
            ],
            "total": 31.0,
        }
    ...
    def ai_review_code(
        ...
        model is absent.
        """
        from kodro.ai.critique import review_program
        from kodro.ai.ollama_client import OllamaClient, OllamaError
    ...
    def swarm_run(self, source: str, lesson_id: str | None = None, n: int = 5) -> dict[str, Any]:
        ...
        """
        from kodro.runtime.swarm import SwarmProgramError, run_swarm
    ...
    def ai_ask(
        ...
        """
        from kodro.ai.ollama_client import OllamaClient, OllamaError
        from kodro.ai.retrieval import build_corpus, grounded_answer, search
...
# Single definition lives in kodro.runtime.session; the Tk app, this bridge
