"""Gymnasium environment over the kodro engine (OPP-3).

:class:`KodroEnv` wraps the SAME deterministic engine that
:mod:`kodro.bench` drives headlessly: the world comes from
``kodro.bench._world_for_seed`` (the seeded obstacle-field domain
randomisation), motion goes through :class:`kodro.engine.rover.Rover`,
and the forward proximity reading comes from
:func:`kodro.engine.sensors.lidar_distance`. Nothing about physics,
...
Reward mirrors :func:`kodro.bench.run_seed`'s success criterion (travel
...
environment by id, ``gymnasium.make("kodro/KodroEnv-v0")``, after
importing :mod:`kodro.envs` (which registers the id); no change to
:mod:`kodro.kodrobench` is required for that.
"""
...
try:
    ...
    raise ImportError(
        "kodro.envs requires the optional 'gymnasium' dependency (which "
        "brings numpy). Install it with: pip install 'kodro[rl]'"
    ) from exc

from kodro.bench import _world_for_seed
from kodro.engine.rover import Rover
from kodro.engine.sensors import LIDAR_MAX_RANGE_M, lidar_distance

#: Registered environment id (``gymnasium.make(ENV_ID)``).
ENV_ID: str = "kodro/KodroEnv-v0"
...
class KodroEnv(gym.Env[NDArray[np.float64], int | np.integer[Any]]):
    ...
    Each ``reset(seed=s)`` rebuilds the exact world :mod:`kodro.bench`
...
def register_env() -> None:
    ...
    gym.register(
        id=ENV_ID,
        entry_point="kodro.envs.kodro_env:KodroEnv",
