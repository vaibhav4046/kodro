...
exposes, then measures three things that matter for a grounded assistant:
  ...
  - invention_rate: fraction of a model's programs that call a command outside
    the build's fitted-command set (the grounding metric, kodro.grounding).
...
number in the docs always traces to a run.
    ...
    python -m kodro.kodrobench --models deterministic,llama3.2:3b --seeds 10 --json results.json
    python -m kodro.kodrobench --models llama3.2:3b --samples 5 --k 1 --json out.json
"""
...
from kodro.bench import run_batch
from kodro.grounding import check_grounding

# v2 adds: per-task train/heldout split (dev_/heldout_ aggregates, free -- no
# extra generation) and an optional pass@k block (kodro.kodrobench.pass_at_k,
...
def generate_leaderboard(results: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "whether it was visible while iterating (see kodro.kodrobench.Task). "
