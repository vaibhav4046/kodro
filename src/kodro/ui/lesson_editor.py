...
:class:`kodro.lessons.schema.Lesson` pydantic model, and writes the
result to ``~/.kodro/custom_lessons/<id>.yaml``. The app loads
...
DEFAULT_CUSTOM_DIR: Path = app_dir() / "custom_lessons"
...
def load_custom_lessons(custom_dir: Path | None = None) -> list[Lesson]:
    """Load every ``*.yaml`` in ``custom_dir`` (default: ``~/.kodro/custom_lessons/``).
