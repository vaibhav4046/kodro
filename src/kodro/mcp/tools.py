...
comes back. :mod:`kodro.mcp.server` owns the JSON-RPC wire format and knows
...
* Only the bundled library is served. Teacher-authored lessons live behind
  :func:`kodro.ui.lesson_editor.load_custom_lessons`, which imports Tk, and
...
from kodro.runtime.session import RunOutcome, run_against_lesson
...
def check_api(params: dict[str, Any]) -> dict[str, Any]:
    ...
    Read out of :data:`kodro.rover_api.__all__` with :mod:`inspect` rather
...
def validate_robot_spec(params: dict[str, Any]) -> dict[str, Any]:
    ...
    not a failure.
    """
    from kodro.interop.urdf_io import kodro_spec_from_krs
    ...
    return {
        ...
        "urdfExportNote": (
            ...
            if urdf_available
            else "URDF export needs the optional 'interop' extra (pip install kodro[interop])."
        ),
    }
...
def prove_contracts(params: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from kodro.prove import build_manifest, load_contracts
