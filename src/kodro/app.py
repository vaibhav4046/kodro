...
shell from :mod:`kodro.ui.main_window` with the editor, simulation,
...
from kodro.memory.hint_engine import HintContext, find_first_hint
...
from kodro.ui import a11y, orbital, sounds
from kodro.ui.console_panel import ConsolePanel, HintCardArea
from kodro.ui.editor_panel import EditorCallbacks, EditorPanel
...
def build_app(
    *,
    ...
    def _explain_code() -> None:
        from kodro.ai import OllamaError, explain_code, is_available
...
A11Y_PATH: Path = app_dir() / "a11y.toml"

#: Default path the teacher progress report is written to.
REPORT_PATH: Path = app_dir() / "progress-report.html"
...
def launch() -> None:
    ...
    from kodro.ui.splash import show_splash
...
WELCOME_SENTINEL: Path = app_dir() / "config.toml"
...
def _maybe_show_welcome(app: App) -> None:
    ...
    if WELCOME_SENTINEL.exists():
        return
    from kodro.ui.welcome_wizard import WelcomeWizard, WizardResult
...
def _default_lesson() -> Lesson:
    ...
    from kodro.lessons.schema import WorldDef as _WD
...
# kodro.runtime.session so a physics or snapshot fix can never land on one
...
def _tween(
    ...
    from kodro.ui.premium import ease_in_out_quintic
...
def _show_trophies(app: App) -> tk.Toplevel:
    ...
    from kodro.memory.achievements import achievement_status
...
# Provide a callable named main() so the existing `python -m kodro`
