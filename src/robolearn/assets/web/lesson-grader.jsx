...
(function () {
  ...
  var LESSON_DATA = {
    "00_first_drive": {"world":{"base":[1,1],"height":6,"obstacles":[],"samples":[],"width":6},"criteria":[{"min_distance_travelled":3},{"no_collisions":true}],"hints":{"onFailure":["Use move_forward to drive the rover ahead. Try move_forward(2).","You can use move_forward more than once to go further."],"onSuccess":[]}},
    "00b_repeat_square": {"world":{"base":[1,1],"height":8,"obstacles":[],"samples":[],"width":8},"criteria":[{"uses_construct":"for"},{"no_collisions":true}],"hints":{"onFailure":["Use a loop: 'for side in range(4):' then indent the steps to repeat.","Inside the loop, move_forward and then turn_right(90) to make each corner."],"onSuccess":[]}},
    "00c_look_first": {"world":{"base":[1,1],"height":6,"obstacles":[{"r":0.4,"x":1,"y":2.2}],"samples":[],"width":6},"criteria":[{"uses_construct":"if"},{"no_collisions":true}],"hints":{"onFailure":["Ask the sensor first: 'if obstacle_ahead():' then turn out of the way.","After the if, drive forward. Keep the move_forward OUTSIDE the if so it always runs."],"onSuccess":[]}},
    "01_hello_rover": {"world":{"base":[1,1],"height":6,"obstacles":[],"samples":[],"width":6},"criteria":[{"max_battery_used":5},{"no_collisions":true}],"hints":{"onFailure":["Check that you call all three functions, in this exact order: move_forward, beep, log.","Make sure each function name is spelled correctly and ends with parentheses."],"onSuccess":[]}},
    "02_move_turn": {"world":{"base":[1,1],"height":6,"obstacles":[],"samples":[[4,4]],"width":6},"criteria":[{"samples_collected":1},{"max_battery_used":15},{"no_collisions":true}],"hints":{"onFailure":["After your first move_forward the rover is east of the base. You need to turn left and drive again to reach (4, 4).","Remember to call collect_sample() once the rover sits on top of the patch."],"onSuccess":[]}},
    "03_sequence": {"world":{"base":[1,1],"height":6,"obstacles":[],"samples":[[1,3]],"width":6},"criteria":[{"samples_collected":1},{"max_battery_used":25},{"no_collisions":true}],"hints":{"onFailure":["Each move_forward distance should match the gap between corners. Sketch the path on paper first.","If the rover stops before the sample, increase the last move_forward distance by one metre."],"onSuccess":[]}},
    "04_selection": {"world":{"base":[1,1],"height":6,"obstacles":[{"r":0.4,"x":3,"y":1}],"samples":[[5,1]],"width":6},"criteria":[{"samples_collected":1},{"no_collisions":true},{"uses_construct":"if"}],"hints":{"onFailure":["The if-statement runs only once. Repeat it (or use a loop) until the rover passes the rock.","Did you call collect_sample() once the rover reaches the patch?"],"onSuccess":[]}},
    "05_iteration": {"world":{"base":[1,5],"height":8,"obstacles":[{"r":0.3,"x":8,"y":5.5}],"samples":[[3,6],[4.5,5.4],[6,6.4]],"width":10},"criteria":[{"samples_collected":3},{"max_battery_used":60},{"no_collisions":true},{"uses_construct":"while"}],"hints":{"onFailure":["Your loop ended early -- have you logged sample_detected() to see when it fires?","Try moving in smaller steps (e.g. move_forward(0.5)) so the rover doesn't overshoot a sample."],"onSuccess":[]}},
    "06_functions": {"world":{"base":[3,3],"height":8,"obstacles":[],"samples":[[5,3],[5,5],[3,5],[3,3]],"width":8},"criteria":[{"samples_collected":4},{"max_battery_used":60},{"uses_construct":"function_def"}],"hints":{"onFailure":["Define hop() once at the top, then call it four times to walk a square.","Add collect_sample() inside hop() so each corner picks up its patch."],"onSuccess":[]}},
    "07_sensors": {"world":{"base":[1,1],"height":6,"obstacles":[],"samples":[[5,1]],"width":6},"criteria":[{"samples_collected":1},{"no_collisions":true}],"hints":{"onFailure":["If the rover hits the wall, your while condition is too generous -- try > 1.0 not > 0.1.","Drive in smaller increments so the loop can react before a collision."],"onSuccess":[]}},
    "08_pathfinding": {"world":{"base":[1,5],"height":10,"obstacles":[{"r":0.4,"x":5,"y":5}],"samples":[[9,5]],"width":10},"criteria":[{"samples_collected":1},{"returns_to_base":true},{"no_collisions":true}],"hints":{"onFailure":["Did the rover get stuck? Add a log() call inside the loop to see which branch fires each iteration.","If the rover never reaches the sample, increase the distance in your else branch."],"onSuccess":[]}},
    "09_recursion": {"world":{"base":[5,5],"height":10,"obstacles":[],"samples":[],"width":10},"criteria":[{"max_battery_used":80},{"no_collisions":true},{"uses_construct":"recursion"}],"hints":{"onFailure":["If the rover runs off the arena, your starting step is too big -- try spiral(2.0).","Don't forget the base case (the if step < 0.5: return) or the function recurses forever."],"onSuccess":[]}},
    "10_optimisation": {"world":{"base":[1,1],"height":8,"obstacles":[],"samples":[[2,6],[5,5],[7,2]],"width":10},"criteria":[{"samples_collected":3},{"returns_to_base":true},{"min_distance_travelled":0},{"max_battery_used":70}],"hints":{"onFailure":["If the rover overshoots, lower the per-step distance from 0.5 m to 0.2 m.","Compare the total distance for at least two different sample orderings."],"onSuccess":[]}},
    "11_decomposition": {"world":{"base":[1,1],"height":10,"obstacles":[],"samples":[[4,1],[4,4]],"width":10},"criteria":[{"samples_collected":2},{"no_collisions":true},{"uses_construct":"function_def"}],"hints":{"onFailure":["You collected the first sample but not the second -- call leg_two() as well.","Each helper should do exactly one leg of the route; then call them in order."],"onSuccess":[]}},
    "12_abstraction": {"world":{"base":[5,1],"height":10,"obstacles":[{"r":0.6,"x":5,"y":4}],"samples":[],"width":10},"criteria":[{"no_collisions":true},{"uses_construct":"if"},{"min_distance_travelled":3}],"hints":{"onFailure":["Check obstacle_ahead() BEFORE you move, then turn_left(90) when it is True.","Wrap the move in an if/else so the rover only drives forward when the way is clear."],"onSuccess":[]}},
    "13_nested_loops": {"world":{"base":[1,1],"height":10,"obstacles":[],"samples":[[3,2],[5,2],[7,2],[3,4],[5,4],[7,4]],"width":10},"criteria":[{"samples_collected":6},{"no_collisions":true},{"uses_construct":"for"}],"hints":{"onFailure":["You collected one row. Add an outer loop so the inner sweep repeats for each row.","After each row, turn and move to the next row before sweeping again."],"onSuccess":[]}},
    "14_counting": {"world":{"base":[1,1],"height":10,"obstacles":[],"samples":[[3,1],[5,1],[7,1]],"width":10},"criteria":[{"samples_collected":3},{"no_collisions":true},{"uses_construct":"while"}],"hints":{"onFailure":["You collected one sample. Put the move/collect/count lines inside a `while count < 3:` loop.","Remember to add 1 to count inside the loop, or it will never reach 3 and will loop forever."],"onSuccess":[]}},
    "15_parameters": {"world":{"base":[1,1],"height":10,"obstacles":[],"samples":[[3,1],[6,1],[8,1]],"width":10},"criteria":[{"samples_collected":3},{"no_collisions":true},{"uses_construct":"function_def"}],"hints":{"onFailure":["The samples are at different gaps, so a fixed 2m hop can't reach them all -- add a `distance` parameter.","Define `def hop(distance):` then call hop(2), hop(3), hop(2) to step between the samples."],"onSuccess":[]}},
  };
