...
(function () {
  ...
  function ped(horiz, dir, lane, speed, s0, color, span) {
    ...
    return {
      ...
      vel: v,
      fade: 1,
      stride: 0.05 + Math.random() * 0.03,
      nextIdle: 4 + Math.random() * 9,
      idleFor: 0
    };
  }
  ...
  function step(dt) {
    ...
    for (let i = 0; i < agents.length; i++) {
      ...
      if (a.kind === 'person') {
        if (a.idleFor > 0) {
          a.idleFor -= dt;
          target = 0; // paused: look around, then carry on
        } else {
          a.nextIdle -= dt;
  ...
  function build(THREE, scene, worldId, opts) {
    ...
    function butterflies(n) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const b = new THREE.Group();
        const wR = new THREE.Mesh(geo, m);
        b.add(wR);
        const wL = new THREE.Mesh(geo, m);
        wL.scale.z = -1;
        b.add(wL);
        extraGrp.add(b);
        ...
        fn: (t, dt) => {
          ...
          for (let i = 0; i < fish.length; i++) {
            ...
            for (let j = 0; j < fish.length; j++) {
              ...
              const d = V3.length();
              if (d > 0.001 && d < 1.6) {
                V3.multiplyScalar((1.6 - d) / (d * d) * 2.2);
                V2.add(V3);
              }
            }
            ...
            const sp = f.v.length();
            if (sp > 4.5) f.v.multiplyScalar(4.5 / sp);else if (sp < 1.8 && sp > 0.001) f.v.multiplyScalar(1.8 / sp);
    ...
    function debris(n) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        rocks.push({
          ...
          r: 18 + Math.random() * 14,
          h: 6 + Math.random() * 7,
          w: (0.015 + Math.random() * 0.015) * (i % 2 ? 1 : -1),
          ph: Math.random() * 6.28,
          rx: 0.2 + Math.random() * 0.3,
          ry: 0.15 + Math.random() * 0.3
        });
      }
      systems.push({
        extra: false,
    ...
    function satellite() {
      ...
      const pan = new THREE.Mesh(new THREE.PlaneGeometry(3.4, 0.8), panelM);
      sat.add(pan);
      extraGrp.add(sat);
      systems.push({
        extra: true,
    ...
    function cat() {
      const furM = new THREE.MeshStandardMaterial({
        color: 0x4a4038,
        roughness: 0.9
      });
      const grp = new THREE.Group();
      ...
      grp.add(torso);
      const body = new THREE.Mesh(new THREE.BoxGeometry(1.05, 0.42, 0.4), furM);
      body.castShadow = true;
      torso.add(body);
      const head = new THREE.Mesh(new THREE.BoxGeometry(0.34, 0.3, 0.32), furM);
      head.position.set(0.62, 0.2, 0);
      head.castShadow = true;
      torso.add(head);
      const earGeo = new THREE.ConeGeometry(0.07, 0.16, 4);
      [-0.1, 0.1].forEach(z => {
        const e = new THREE.Mesh(earGeo, furM);
        e.position.set(0.6, 0.42, z);
        torso.add(e);
      });
      ...
      systems.push({
        ...
        fn: (t, dt) => {
          ...
          if (st.mode === 'walk') {
            ...
            legs[0].rotation.z = lp;
            legs[1].rotation.z = -lp;
            legs[2].rotation.z = -lp;
            legs[3].rotation.z = lp;
            tail.rotation.x = Math.sin(t * 2.5) * 0.2;
          } else {
            st.sitT -= dt;
            st.sitLean += (0.5 - st.sitLean) * k;
            torso.position.y += (0.46 - torso.position.y) * k;
            legs[0].rotation.z *= 0.9;
            legs[1].rotation.z *= 0.9;
...
(function () {
  ...
  function Viewport3D({
    ...
    useEffect(() => {
      ...
      } else if (_sid === 'warehouse') {
        scene.background = new THREE.Color(0x383b42);
        scene.fog = new THREE.Fog(0x383b42, 50, 200);
      } else if (_sid === 'debug_grid') {
      ...
      function mkPerson(shirt) {
        ...
        if (!personShared.torso) {
          ...
          personShared.legM = new THREE.MeshStandardMaterial({
            color: 0x2f3646,
            roughness: 0.9
          });
          personShared.skinM = new THREE.MeshStandardMaterial({
            color: 0xe8c9a8,
            roughness: 0.7
          });
        }
        ...
        const torso = new THREE.Mesh(personShared.torso, shirtM);
        torso.position.y = 1.7;
        torso.castShadow = true;
        const head = new THREE.Mesh(personShared.head, personShared.skinM);
        head.position.y = 2.5;
        head.castShadow = true;
        p.add(torso);
        p.add(head);
      ...
      const applyFade = (mesh, a) => {
        ...
        mesh.visible = f > 0.02;
        if (f < 1) mesh.scale.setScalar(Math.max(f, 0.001));else if (mesh.scale.x !== 1) mesh.scale.setScalar(1);
      };
      ...
      function buildCity() {
        ...
        try {
          const sTrunkM = new THREE.MeshStandardMaterial({
            color: 0x6b4f2c,
            roughness: 1
          });
          const sLeafM = new THREE.MeshStandardMaterial({
            color: 0x3a6b2a,
            roughness: 1,
            flatShading: true
          });
          [[ROADW / 2 + 6, 10], [ROADW / 2 + 6, -10], [ROADW / 2 + 6, 25], [-(ROADW / 2 + 6), -25], [-(ROADW / 2 + 6), 15]].forEach(p => {
            ...
            const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.28, 3, 6), sTrunkM);
            trunk.position.y = 1.5;
            trunk.castShadow = true;
            t.add(trunk);
            const fol = new THREE.Mesh(new THREE.IcosahedronGeometry(1.4, 0), sLeafM);
            fol.position.y = 3.8;
            fol.castShadow = true;
            t.add(fol);
      ...
      function buildRoom() {
        ...
        try {
          const winFrameM = new THREE.MeshStandardMaterial({
            color: 0xe8e0d0,
            roughness: 0.8
          });
          const winGlassM = new THREE.MeshStandardMaterial({
            color: 0x8ec5e8,
            roughness: 0.2,
            metalness: 0.1,
            emissive: 0x4a7a9a,
          ...
          winGlass.position.set(R - 0.35, 8, -4);
          scene.add(winGlass);
          const barH = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.2, 5.2), winFrameM);
          barH.position.set(R - 0.32, 8, -4);
          scene.add(barH);
          const barV = new THREE.Mesh(new THREE.BoxGeometry(0.08, 5.2, 0.2), winFrameM);
          barV.position.set(R - 0.32, 8, -4);
          scene.add(barV);
          const fT = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.3, 7.3), winFrameM);
          fT.position.set(R - 0.33, 10.6, -4);
          scene.add(fT);
          const fB = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.3, 7.3), winFrameM);
          fB.position.set(R - 0.33, 5.4, -4);
          scene.add(fB);
          const fL = new THREE.Mesh(new THREE.BoxGeometry(0.1, 5.5, 0.3), winFrameM);
          fL.position.set(R - 0.33, 8, -7.5);
          scene.add(fL);
          const fR = new THREE.Mesh(new THREE.BoxGeometry(0.1, 5.5, 0.3), winFrameM);
          fR.position.set(R - 0.33, 8, -0.5);
          scene.add(fR);
        } catch (e) {
          if (window.console) console.warn('Viewport3D window failed:', e);
        }
        ...
        try {
          const rugTex = typeof document !== 'undefined' && function () {
            ...
            c2.strokeStyle = 'rgba(180,120,80,0.35)';
            c2.lineWidth = 2;
            c2.strokeRect(6, 6, 116, 116);
            c2.strokeRect(16, 16, 96, 96);
            for (let i = 0; i < 6; i++) {
              c2.beginPath();
              c2.arc(64, 64, 18 + i * 10, 0, 6.283);
              c2.strokeStyle = 'rgba(160,90,70,' + (0.3 - i * 0.04) + ')';
              c2.stroke();
            }
            return new THREE.CanvasTexture(cv);
          }();
          const rugM = new THREE.MeshStandardMaterial({
            color: 0x9a5f54,
            roughness: 1
          });
          if (rugTex) {
            rugM.map = rugTex;
            rugM.needsUpdate = true;
          }
          const rug = new THREE.Mesh(new THREE.PlaneGeometry(10, 7), rugM);
          rug.rotation.x = -Math.PI / 2;
          rug.position.set(-14.1, 0.03, -11.4);
          scene.add(rug);
        } catch (e) {
          if (window.console) console.warn('Viewport3D rug failed:', e);
        }
        ...
        try {
          ...
          for (let s = 0; s < 2; s++) {
            ...
            for (let b = 0; b < 6; b++) {
              ...
              const book = new THREE.Mesh(new THREE.BoxGeometry(bw, bh, 0.7), new THREE.MeshStandardMaterial({
          ...
          if (window.console) console.warn('Viewport3D books failed:', e);
        }
        ...
        try {
          ...
          ceilArm.position.set(0, wallH - 0.8, 0);
          scene.add(ceilArm);
          const ceilBulb = new THREE.Mesh(new THREE.SphereGeometry(0.5, 16, 12), new THREE.MeshStandardMaterial({
            color: 0xfff5d0,
            emissive: 0xffe8a0,
            emissiveIntensity: 0.6
          }));
          ceilBulb.position.set(0, wallH - 1.8, 0);
          scene.add(ceilBulb);
        } catch (e) {
          if (window.console) console.warn('Viewport3D ceiling light failed:', e);
        }
      ...
      function buildIndoor(kind) {
        ...
        const wallM = new THREE.MeshStandardMaterial({
          color: wallCol,
          roughness: 0.92,
          metalness: kind === 'warehouse' ? 0.2 : 0.05,
          side: THREE.DoubleSide
        });
        const mkWall = (w, x, z, ry) => {
          const ww = new THREE.Mesh(new THREE.BoxGeometry(w, wallH, 0.6), wallM);
        ...
        const crateM = new THREE.MeshStandardMaterial({
          color: 0xb07a3a,
          roughness: 0.8
        });
        const rackM = new THREE.MeshStandardMaterial({
          color: 0x3a4654,
          roughness: 0.5,
          metalness: 0.6
        });
        const markM = new THREE.MeshStandardMaterial({
          color: 0x5ce0d8,
          emissive: 0x5ce0d8,
          emissiveIntensity: 0.6
        });
        obstacles.forEach(o => {
          const px = o.x * SCALE,
            pz = -o.y * SCALE,
            r = Math.max(0.8, o.r * SCALE);
          if (kind === 'lab') {
            const top = new THREE.Mesh(new THREE.BoxGeometry(r * 2.2, 0.5, r * 1.4), benchM);
            top.position.set(px, 2.0, pz);
            top.castShadow = true;
            top.receiveShadow = true;
            scene.add(top);
            const inst = new THREE.Mesh(new THREE.BoxGeometry(r * 0.8, r * 0.7, r * 0.8), steelM);
            inst.position.set(px, 2.25 + r * 0.35, pz);
            inst.castShadow = true;
            scene.add(inst);
            [[-1, -0.6], [1, -0.6], [-1, 0.6], [1, 0.6]].forEach(c => {
              const lg = new THREE.Mesh(new THREE.BoxGeometry(0.18, 2.0, 0.18), steelM);
              lg.position.set(px + c[0] * r, 1.0, pz + c[1] * r);
              scene.add(lg);
            });
          } else if (kind === 'warehouse') {
            // An open shelving rack (posts + shelves + crates), size-capped so a
            // big obstacle radius does not become a giant solid slab.
            const rw = Math.min(2.6, 1.2 + r * 0.5),
              rd = Math.min(1.8, 1.0 + r * 0.3),
              h = Math.min(9, 5 + r * 0.8);
            const frame = new THREE.Group();
            [[-1, -1], [1, -1], [-1, 1], [1, 1]].forEach(c => {
              const post = new THREE.Mesh(new THREE.BoxGeometry(0.18, h, 0.18), rackM);
              post.position.set(c[0] * rw, h / 2, c[1] * rd);
              post.castShadow = true;
              frame.add(post);
            });
            for (let s = 0; s < 3; s++) {
              const y = 0.5 + s * (h / 3);
              const shelf = new THREE.Mesh(new THREE.BoxGeometry(rw * 2 + 0.2, 0.12, rd * 2 + 0.2), rackM);
              shelf.position.set(0, y, 0);
              shelf.receiveShadow = true;
              frame.add(shelf);
              const cr = new THREE.Mesh(new THREE.BoxGeometry(rw * 1.3, 0.9, rd * 1.3), crateM);
              cr.position.set(0, y + 0.55, 0);
              cr.castShadow = true;
              frame.add(cr);
            }
            ...
            const m = new THREE.Mesh(new THREE.ConeGeometry(r * 0.6, r * 1.4, 4), markM);
        ...
        if (KAr && kind !== 'debug_grid') {
          KAr.list().forEach((ag, i) => {
            if (ag.kind !== 'person') return;
            const pr = mkPerson(ag.color != null ? ag.color : 0x9fb0c4);
            scene.add(pr);
            agents.push({
              mesh: pr,
              update: () => {
      ...
      if (_siteId === 'lab' || _siteId === 'warehouse' || _siteId === 'debug_grid') buildIndoor(_siteId);else if (id === 'city') buildCity();else if (id === 'room') buildRoom();else {
        ...
        try {
          ...
          if (_rt === 'rover' && (id === 'earth' || id === 'mars' || id === 'space')) {
            ...
            const antM = new THREE.MeshStandardMaterial({
              color: 0x9aa0ad,
              roughness: 0.3,
              metalness: 0.7
            });
            ...
            if (panelTex) {
              panelM.map = panelTex;
              panelM.needsUpdate = true;
            }
            const panel = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.04, 0.5), panelM);
            panel.position.set(-0.9, 0.3, -0.5);
            panel.rotation.z = 30 * Math.PI / 180;
            panel.castShadow = true;
            scene.add(panel);
          }
        } catch (e) {
          if (window.console) console.warn('Viewport3D base camp failed:', e);
        }
      }
      ...
      const tick = () => {
        ...
        if (ambient && !reduce) {
          try {
            ...
            if (window.console) console.warn('KodroAmbient update failed; ambient life stopped:', e);
            ambient = null;
          }
        }
...
(function () {
  ...
  window.KodroCommands = {
    ...
    driveCheck: function (robot, cmdName) {
      ...
      if (!robot) return {
        ok: true
      }; // no build context (headless QA): do not gate
      const fitted = fittedParts(robot) || [];
      const hasDrive = DRIVE_ACTUATORS.some(function (a) {
        return fitted.indexOf(a) >= 0;
      });
      if (hasDrive) return {
        ok: true
      };
    ...
    availability: function (robot) {
      const out = BASE_COMMANDS.map(function (c) {
        ...
        return {
          name: c,
          available: dr.ok,
          requires: dr.ok ? null : 'drive',
          reason: dr.ok ? null : dr.reason
        };
      });
    ...
    try {
      for (;;) {
        ...
        if ((ev.type === 'move' || ev.type === 'turn') && window.KodroCommands) {
          const cmdName = ev.type === 'move' ? ev.dir < 0 ? 'move_backward' : 'move_forward' : ev.deg < 0 ? 'turn_left' : 'turn_right';
...
(function () {
  ...
  var LESSON_DATA = {
    ...
    "00b_repeat_square": {
      ...
      "hints": {
        "onFailure": ["Use a loop: 'for side in range(4):' then indent the steps to repeat.", "Inside the loop, move_forward and then turn_right(90) to make each corner."],
        "onSuccess": []
      }
    },
    "00c_look_first": {
      ...
      "hints": {
        "onFailure": ["Ask the sensor first: 'if obstacle_ahead():' then turn out of the way.", "After the if, drive forward. Keep the move_forward OUTSIDE the if so it always runs."],
        "onSuccess": []
      }
    },
    "01_hello_rover": {
      ...
      "hints": {
        "onFailure": ["Check that you call all three functions, in this exact order: move_forward, beep, log.", "Make sure each function name is spelled correctly and ends with parentheses."],
        "onSuccess": []
      }
    },
    "02_move_turn": {
    ...
    "03_sequence": {
      ...
      "hints": {
        "onFailure": ["Each move_forward distance should match the gap between corners. Sketch the path on paper first.", "If the rover stops before the sample, increase the last move_forward distance by one metre."],
        "onSuccess": []
      }
    },
    "04_selection": {
    ...
    "05_iteration": {
      ...
      "hints": {
        "onFailure": ["Your loop ended early -- have you logged sample_detected() to see when it fires?", "Try moving in smaller steps (e.g. move_forward(0.5)) so the rover doesn't overshoot a sample."],
        "onSuccess": []
      }
    },
    "06_functions": {
      ...
      "hints": {
        "onFailure": ["Define hop() once at the top, then call it four times to walk a square.", "Add collect_sample() inside hop() so each corner picks up its patch."],
        "onSuccess": []
      }
    },
    "07_sensors": {
    ...
    "09_recursion": {
      ...
      "criteria": [{
        ...
        "uses_construct": "recursion"
      }],
      "hints": {
        "onFailure": ["If the rover runs off the arena, your starting step is too big -- try spiral(2.0).", "Don't forget the base case (the if step < 0.5: return) or the function recurses forever."],
        "onSuccess": []
      }
    },
    "10_optimisation": {
    ...
    "11_decomposition": {
      ...
      "hints": {
        "onFailure": ["You collected the first sample but not the second -- call leg_two() as well.", "Each helper should do exactly one leg of the route; then call them in order."],
        "onSuccess": []
      }
    },
    "12_abstraction": {
      ...
      "hints": {
        "onFailure": ["Check obstacle_ahead() BEFORE you move, then turn_left(90) when it is True.", "Wrap the move in an if/else so the rover only drives forward when the way is clear."],
        "onSuccess": []
      }
    },
    "13_nested_loops": {
      ...
      "hints": {
        "onFailure": ["You collected one row. Add an outer loop so the inner sweep repeats for each row.", "After each row, turn and move to the next row before sweeping again."],
        "onSuccess": []
      }
    },
    "14_counting": {
      ...
      "hints": {
        "onFailure": ["You collected one sample. Put the move/collect/count lines inside a `while count < 3:` loop.", "Remember to add 1 to count inside the loop, or it will never reach 3 and will loop forever."],
        "onSuccess": []
      }
    },
    "15_parameters": {
  ...
  function runProgram(entry, source, terrain) {
    ...
    if (!window.RoverLang) {
      return {
        ...
        error: {
          kind: 'runtime',
          message: 'interpreter not loaded',
          line: null
        }
      };
    }
      ...
      for (;;) {
        ...
        if (++guard > EVENT_CAP) {
          return {
            ...
            error: {
              kind: 'timeout',
              message: 'program did not finish (too many steps)',
              line: null
            }
          };
        }
        ...
        switch (ev.type) {
          ...
          default:
            break;
          // 'step' bookkeeping events are not API calls
        }
      }
    } catch (e) {
      return {
  ...
  function walkNode(node, fn) {
    ...
    if (node.kind) {
      switch (node.kind) {
        case 'for':
          walkNode(node.iter, fn);
          walkList(node.body, fn);
          break;
        case 'while':
        ...
        case 'def':
          ...
          break;
        case 'return':
          if (node.expr) walkNode(node.expr, fn);
          break;
        case 'assign':
        case 'augassign':
          walkNode(node.expr, fn);
          break;
        case 'expr':
          walkNode(node.expr, fn);
          break;
  ...
  var CONSTRUCT_TESTS = {
    ...
    'function_def': function (n) {
      return n.kind === 'def';
    },
    'function_call': function (n) {
      return n.k === 'call';
    },
    ...
    'assignment': function (n) {
      return n.kind === 'assign' || n.kind === 'augassign';
    },
    'logical': function (n) {
      return n.k === 'and' || n.k === 'or';
    },
    'return': function (n) {
  ...
  function returnsToBase(agg, entry) {
    ...
    var bx = entry.world.base[0],
      by = entry.world.base[1];
  ...
  function checkCriterion(criterion, agg, entry, source) {
    if (criterion.samples_collected !== undefined && agg.samplesCollected < criterion.samples_collected) {
    ...
    if (criterion.max_battery_used !== undefined && agg.batteryUsedPct > criterion.max_battery_used) {
      return 'Battery used ' + agg.batteryUsedPct.toFixed(1) + '% ' + '(limit ' + Number(criterion.max_battery_used).toFixed(1) + '%).';
    }
    ...
    if (criterion.min_distance_travelled !== undefined && agg.distanceTravelledM < criterion.min_distance_travelled) {
      return 'Travelled ' + agg.distanceTravelledM.toFixed(1) + ' m ' + '(minimum ' + Number(criterion.min_distance_travelled).toFixed(1) + ' m).';
    }
    return null;
  }
  ...
  function firstHint(entry, passed) {
    ...
    return {
      ruleName: passed ? 'lesson_on_success' : 'lesson_on_failure',
      message: bank[0]
    };
  }
  ...
  window.KodroLessonGrader = {
    grade: function (lesson, source) {
      return new Promise(function (resolve) {
        resolve(gradeSync(lesson, source));
      });
    },
    gradeSync: gradeSync,
...
(function () {
  ...
  function KodroOnboarding({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "konb-shell"
    }, /*#__PURE__*/React.createElement("header", {
      className: "konb-topbar"
    }, /*#__PURE__*/React.createElement("div", {
      className: "konb-wordmark"
    }, /*#__PURE__*/React.createElement("div", {
      className: "wm-mark"
      ...
      className: "konb-skip",
      onClick: onClose
    }, "Skip to studio")), /*#__PURE__*/React.createElement("main", {
      className: "konb-body"
    }, step === 0 && /*#__PURE__*/React.createElement("section", {
      className: "konb-hero",
      "aria-labelledby": "konb-h1"
    }, /*#__PURE__*/React.createElement("div", {
      className: "konb-lede"
    }, /*#__PURE__*/React.createElement("span", {
      ...
      key: i
    }, c))), /*#__PURE__*/React.createElement(Steps, {
      current: 0
    }), /*#__PURE__*/React.createElement("div", {
      ...
      onClick: onClose
    }, "Skip to studio"))), /*#__PURE__*/React.createElement("div", {
      className: "konb-orbit",
      "aria-hidden": "true"
    }, /*#__PURE__*/React.createElement("div", {
      className: "ring r1"
    }), /*#__PURE__*/React.createElement("div", {
      ...
      className: "konb-core"
    }, MARK))), step === 1 && /*#__PURE__*/React.createElement("section", {
      className: "konb-panel",
      ...
      className: "konb-sub"
    }, "The assistant picks a world that suits your robot. Test it there first, then try the others from the studio."), /*#__PURE__*/React.createElement("div", {
      className: "konb-rec"
    }, /*#__PURE__*/React.createElement("div", {
      className: "rec-badge",
      "aria-hidden": "true"
      ...
      className: "rec-label"
    }, "Recommended world"), /*#__PURE__*/React.createElement("div", {
      ...
      className: "konb-btn primary",
      onClick: enterStudio
    }, "Enter studio ", /*#__PURE__*/React.createElement("span", {
      className: "arr",
      "aria-hidden": "true"
    }, "\u2192")))))));
  }
...
(function () {
  ...
  function useVibeChat(opts) {
    ...
    const currentLessonIdRef = opts.currentLessonIdRef || {
      current: null
    };
  ...
  function useEditorApply(opts) {
    ...
    const showToast = opts.showToast || function () {};
    const addConsole = opts.addConsole || function () {};
    const prefersReducedMotion = opts.prefersReducedMotion || function () {
      return false;
    };
    ...
    function typewriteCode(codeText) {
      ...
      if (prefersReducedMotion() || codeText.length > 4000) {
        setCode(codeText);
        return;
      }
      let i = 0;
  ...
  function useSimEngine(deps) {
    ...
    async function animateMove(ev) {
      ...
      const accelFrac = prof.accelFrac,
        brakeFrac = prof.brakeFrac,
        cruiseFrac = prof.cruiseFrac,
        profileArea = prof.profileArea;
...
(function () {
  ...
  function BuildModal({
    ...
    }, "Type a budget and the built-in AI plans a real robot you could build and program, showing which real part does each job your simulated robot does. Nothing is ordered, and nothing goes online."), robotSpec && /*#__PURE__*/React.createElement("p", {
      className: "build-active",
      "data-build-active": "1"
    }, "Pricing your current robot: ", /*#__PURE__*/React.createElement("b", null, robotSpec.name || 'My Robot'), ' · ', robotSpec.type || 'rover', ' · ', [].concat(robotSpec.sensors || [], robotSpec.actuators || []).join(', ') || 'no parts fitted'), /*#__PURE__*/React.createElement("div", {
  ...
  function SwarmModal({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("svg", {
        ...
        className: "build-note"
      }, swarmData.n, " rovers ran the same program from different starting points. Same code on every rover, no leader telling them what to do, and a pattern still forms. All offline."));
    })())));
  }
  ...
  function AskModal({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "build-note"
    }, "Install the free on-device AI (Ollama) for a written answer. The matching lesson notes above work without it.")))));
  }
  ...
  function MemoryModal({
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "rl-label"
    }, "Notes from past runs"), (window.KodroMemory ? window.KodroMemory.reflections() : []).length ? /*#__PURE__*/React.createElement("ul", {
      className: "mem-list"
      ...
      className: "rl-label"
    }, "Skill library. Programs that worked, saved for reuse"), /*#__PURE__*/React.createElement("button", {
  ...
  function VibeModal({
    ...
    const ctxChip = vibeContext && vibeContext.reflection ? /*#__PURE__*/React.createElement("div", {
      className: "vibe-ctx",
      title: "Learned from your past runs in this world; the AI is reminded of it when it writes code for you"
    }, /*#__PURE__*/React.createElement("span", {
      className: "vibe-ctx-tag"
    }, "Learned from past runs", vibeContext.world ? ' · ' + vibeContext.world : ''), /*#__PURE__*/React.createElement("span", {
  ...
  function BlocksModal({
    ...
    }, "Turns into real Python in the editor. Nothing runs until you press Run."), /*#__PURE__*/React.createElement("button", {
      className: "ctrl ctrl-run",
      disabled: !blocks.length,
...
(function () {
  ...
  function App() {
    ...
    const {
      ...
      setToasts,
      showToast
    } = window.KodroHooks && window.KodroHooks.useConsoleToast ? window.KodroHooks.useConsoleToast() : {
      consoleLines: [],
    ...
    const {
      ...
      openProjectClick,
      onProjectFilePicked
    } = window.KodroHooks && window.KodroHooks.useProjectIO ? window.KodroHooks.useProjectIO({
      setSettingsOpen,
      showToast,
      addConsole
    }) : {
    ...
    const {
      typewriteCode,
      applyProgramText
    } = window.KodroHooks && window.KodroHooks.useEditorApply ? window.KodroHooks.useEditorApply({
      lessonBuffers,
      programs,
      setLessonBuffers,
      setPrograms,
      ...
      typewriteCode: function () {},
      applyProgramText: function () {}
    };
    ...
    const {
      ...
      vibePrompt: '',
      setVibePrompt: function () {},
      vibeBusy: false,
      setVibeBusy: function () {},
      vibeError: null,
      setVibeError: function () {},
      vibeMsgs: [],
      setVibeMsgs: function () {},
      vibeEndRef: {
        current: null
      },
      vibeLive: '',
      setVibeLive: function () {},
      vibeCancelRef: {
        current: false
      },
      vibeSend: function () {},
    ...
    const {
      ...
      pickModel,
      refreshAiStatus
    } = window.KodroHooks ? window.KodroHooks.useAiStatus(vibeOpen) : {
      aiInfo: {},
      pickModel: function () {},
    ...
    const {
      ...
      openBuildReal,
      adoptPlanParts
    } = window.KodroHooks && window.KodroHooks.useBuild ? window.KodroHooks.useBuild({
      setRobotLabOpen,
      showToast
    }) : {
      buildOpen: false,
      setBuildOpen: function () {},
      buildBudget: '30',
      setBuildBudget: function () {},
      buildGoal: '',
      setBuildGoal: function () {},
      buildBusy: false,
      buildPlan: null,
      buildErr: null,
      runBuild: function () {},
      specGoalText: function () {
        return '';
      },
      openBuildReal: function () {},
      adoptPlanParts: function () {}
    };
    useEffect(() => {
      const onRobot = e => {
    ...
    const {
      ...
      teacherData,
      openTeacher
    } = window.KodroHooks && window.KodroHooks.useTeacher ? window.KodroHooks.useTeacher({
      setSettingsOpen
    }) : {
      teacherOpen: false,
      setTeacherOpen: function () {},
      teacherData: null,
    ...
    const {
      ...
      swarmData,
      runSwarm
    } = window.KodroHooks && window.KodroHooks.useSwarm ? window.KodroHooks.useSwarm({
      code,
      currentLessonId,
      addConsole
    }) : {
      swarmOpen: false,
      setSwarmOpen: function () {},
      swarmBusy: false,
      swarmData: null,
      runSwarm: function () {}
    };
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "icon-btn",
      title: "Settings",
      "aria-label": "Settings \u2014 theme, sound, readable text, and teacher tools",
      ...
      className: 'panel tele-panel' + (teleCollapsed ? ' tele-collapsed' : ''),
      style: {
        gridColumn: 5
      }
    }, /*#__PURE__*/React.createElement("div", {
